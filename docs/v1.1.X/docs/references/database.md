...
The database is mostly used by the {{v11X.lapi.htmlname}} but also by {{v11X.cli.user_guide}} for some tasks.
...
The database configuration can be found in the `crowdsec` configuration file (default: {{v11X.config.crowdsec_config_file}}).
...
{{v11X.crowdsec.name}} uses the [ent framework](https://entgo.io/) to manage the database.
