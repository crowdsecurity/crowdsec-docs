{{v11X.crowdsec.name}} can expose a {{v11X.prometheus.htmlname}} endpoint for collection (on `http://127.0.0.1:6060/metrics` by default).

The goal of this endpoint, besides the usual resources consumption monitoring, aims at offering a view of {{v11X.crowdsec.name}} "applicative" behavior :
...
All the counters are "since {{v11X.crowdsec.name}} start".
...
#### Info
 ...
 - `cs_info` : Information about {{v11X.crowdsec.name}} (software version)
