...
While {{v11X.crowdsec.name}} can be used to monitor "live" logs, it can as well be used on cold logs.
...
And we can then query the local api (while letting the {{v11X.crowdsec.name}} running) :
```bash
