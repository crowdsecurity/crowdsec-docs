...
{{v11X.hub.htmlname}} allows you to find needed collections.
...
<details>
  <summary>{{v11X.cli.name}} collection install example</summary>
...
```bash
$ sudo {{v11X.cli.bin}} collections list
```
...
```bash
$ sudo {{v11X.cli.bin}} hub update
$ sudo {{v11X.cli.bin}} collections upgrade crowdsecurity/sshd
```
...
[Link to collections reference documentation](/Crowdsec/v11/references/collections/)
