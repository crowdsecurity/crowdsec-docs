...
The whitelist can be based on several criteria :
 ...
 - a list of {{v11X.expr.htmlname}} expressions : if any expression returns true, event is whitelisted
...
and reload {{v11X.crowdsec.name}} : `sudo systemctl reload crowdsec`
...
again, let's restart {{v11X.crowdsec.name}} !
...
After reloading {{v11X.crowdsec.name}}, and launching (again!) nikto :
