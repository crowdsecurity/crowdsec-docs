...
```bash
sudo {{v11X.cli.bin}} hub list
```

On the machine where you deployed {{v11X.crowdsec.name}}, type `sudo {{v11X.cli.bin}} hub list` to see install configurations.
This list represents the parsers, scenarios and/or collections that you deployed. They represent what your {{v11X.crowdsec.name}} setup can read (logs) and detect (scenarios). `sudo {{v11X.cli.bin}} hub list -a` will list all available configurations in the hub.


Check [{{v11X.cli.name}} configuration](/Crowdsec/v11/user_guide/cscli/) management for more !
...
`configuration_type` can be:
 ...
 - [`parsers`](https://docs.crowdsec.net/Crowdsec/v11/references/parsers/)

 - [`scenarios`](https://docs.crowdsec.net/Crowdsec/v11/references/scenarios/)

 - [`postoverflows`](https://docs.crowdsec.net/Crowdsec/v11/references/postoverflows/)

 - [`collections`](https://docs.crowdsec.net/Crowdsec/v11/references/collections/)


They can be found and browsed on the {{v11X.hub.htmlname}}
...
If you just deployed {{v11X.crowdsec.name}}, the list might be empty, but don't worry, it simply means you haven't yet been attacked, congrats!

Check [{{v11X.cli.name}} decisions](/Crowdsec/v11/user_guide/decision_management/) management for more !
...
There are different decisions `SOURCE`:
  ...
  - csli : decisions added via `sudo {{v11X.cli.bin}} decisions add`
...
The metrics displayed are extracted from {{v11X.crowdsec.name}} prometheus.
...
For more advanced metrics understanding, [take a look at the dedicated prometheus documentation](/Crowdsec/v11/observability/prometheus/).
...
A docker metabase {{v11X.metabase.Htmlname}} container can be deployed with `cscli dashboard`.
...
By default, one routine of each exists (should be enough to handle ~1K EP/s), and can be changed in `crowdsec_service` of the main configuration file via the [parser_routines](/Crowdsec/v11/references/crowdsec-config/#parser_routines), [buckets_routines](/Crowdsec/v11/references/crowdsec-config/#buckets_routines) and [output_routines](/Crowdsec/v11/references/crowdsec-config/#output_routines) directives.

Please keep in mind that thanks to the [http API]({{v11X.lapi.swagger}}), the workload of log parsing can be splitted amongst several agents pushing to a single {{v11X.lapi.Htmlname}}.
