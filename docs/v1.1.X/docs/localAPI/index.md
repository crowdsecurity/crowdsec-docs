...
The Local API (LAPI) is a core component of {{v11X.crowdsec.name}} and has a few essential missions :
...
There are two kinds of authentication to the local API :
 ...
 - {{v11X.bouncers.Name}} : they authenticate with a simple API key and can only read decisions
...
and keep the generated API token to use it in your {{v11X.bouncers.Name}} configuration file.
...
Modify the [`listen_uri` option](/Crowdsec/v11/references/crowdsec-config/#listen_uri) in the main configuration file.
Then see [how to configure your crowdsec to use a remote API](/Crowdsec/v11/localAPI/#register-to-a-remote-api-server).
...
You can see how to configure SSL on your local API [here](/Crowdsec/v11/references/crowdsec-config/#tls).


See the [Local API public documentation]({{v11X.lapi.swagger}}).
