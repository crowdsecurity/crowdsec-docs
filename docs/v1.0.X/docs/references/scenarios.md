...
Scenarios receive {{v10X.event.htmlname}}(s) and can produce {{v10X.alert.htmlname}}(s) using the [leaky bucket](https://en.wikipedia.org/wiki/Leaky_bucket) algorithm.

As an {{v10X.event.htmlname}} can be the representation of a log line, or an overflow, it  allows scenarios to process both logs or overflows to allow inference.
...
Scenarios can be of different types (leaky, trigger, counter), and are based on various factors, such as :
  ...
  - the characteristic(s) of eligible {{v10X.event.htmlname}}(s) : "log type XX with field YY set to ZZ"
...
Behind the scenes, {{v10X.crowdsec.name}} is going to create one or more buckets when events with matching characteristics arrive to the scenario. When any of these buckets overflows, the scenario has been triggered.
...
{{v10X.crowdsec.name}} can be used not only to process live logs, but as well to process "cold" logs (think forensics).
...
Defines the type of the bucket. Currently three types are supported :
 ...
 - `leaky` : a [leaky bucket](https://en.wikipedia.org/wiki/Leaky_bucket) that must be configured with a {{v10X.capacity.htmlname}} and a {{v10X.leakspeed.htmlname}}
 - `trigger` : a bucket that overflows as soon as an event is poured (it's like a leaky bucket is a capacity of 0)
 - `counter` : a bucket that only overflows every {{v10X.duration.htmlname}}. It's especially useful to count things.
...
`filter` must be a valid {{v10X.expr.htmlname}} expression that will be evaluated against the {{v10X.event.htmlname}}.
...
an {{v10X.expr.htmlname}} that must return a string. This string will be used as to partition the buckets.
...
an {{v10X.expr.htmlname}} that must return a string. The event will be poured **only** if the string is not already present in the bucket.
...
By default, a bucket holds {{v10X.capacity.htmlname}} events "in memory".
...
`overflow_filter` is an {{v10X.expr.htmlname}} that is run when the bucket overflows.
...
This section is only relevant when `cscli` is used to install scenario from hub, as ill download the `source_url` and store it to `dest_file`. When the scenario is not installed from the hub, {{v10X.crowdsec.name}} won't download the URL, but the file must exist for the scenario to be loaded correctly.
...
{{v10X.crowdsec.name}} has a notion of format support for parsers & scenarios for compatibility management.
...
While most scenarios might focus on Ips, {{v10X.crowdsec.name}} and {{v10X.bouncers.name}} can work with any scope.
