...
The parser nodes are processed sequentially based on the alphabetical order of {{v10X.stage.htmlname}} and subsequent files.
If the node is considered successful (grok is present and returned data or no grok is present) and "onsuccess" equals to `next_stage`, then the {{v10X.event.name}} is moved to the next stage.
...
The `grok` structure in a node represent a regular expression with capture group (grok pattern) that must be applied on a field of {{v10X.event.name}}.

The pattern can : 

 - be imported by name (if present within the core of {{v10X.crowdsec.name}})
 - defined in place
...
if set to `next_stage` and the node is considered successful, the {{v10X.event.name}} will be moved directly to the next stage without processing other nodes in the current stage. _note: if it's a parser tree, and a "leaf" node succeeds, it is the parent's "onsuccess" that is evaluated._
...
The target aims at being any part of the {{v10X.event.htmlname}} object, and can be expressed in different ways :
...
#### `value`
 ...
 The source itself can be either a static value, or an {{v10X.expr.htmlname}} result :
...
This section is only relevant when `cscli` is used to install parser from hub, as it will download the `source_url` and store it to `dest_file`. When the parser is not installed from the hub, {{v10X.crowdsec.name}} won't download the URL, but the file must exist for the parser to be loaded correctly.
