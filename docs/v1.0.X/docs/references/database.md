...
The database is mostly used by the {{v10X.lapi.htmlname}} but also by {{v10X.cli.user_guide}} for some tasks.
...
The database configuration can be found in the `crowdsec` configuration file (default: {{v10X.config.crowdsec_config_file}}).
...
{{v10X.crowdsec.name}} uses the [ent framework](https://entgo.io/) to manage the database.
