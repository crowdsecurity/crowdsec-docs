...
The Local API (LAPI) is a core component of {{v10X.crowdsec.name}} and has a few essential missions :
...
There are two kinds of authentication to the local API :
 ...
 - {{v10X.bouncers.Name}} : they authenticate with a simple API key and can only read decisions
...
and keep the generated API token to use it in your {{v10X.bouncers.Name}} configuration file.
...
See the [Local API public documentation]({{v10X.lapi.swagger}}).
