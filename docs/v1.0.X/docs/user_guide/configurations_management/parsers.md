{{v10X.hub.htmlname}} allows you to find needed parsers.
...
{{v10X.parsers.Htmlname}} are yaml files in `{{v10X.config.crowdsec_dir}}parsers/<STAGE>/parser.yaml`.
...
```bash
$ sudo {{v10X.cli.bin}} parsers upgrade crowdsecurity/sshd-logs
```
