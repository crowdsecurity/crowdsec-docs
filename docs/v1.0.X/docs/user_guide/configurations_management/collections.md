...
{{v10X.hub.htmlname}} allows you to find needed collections.
...
<details>
  <summary>{{v10X.cli.name}} collection install example</summary>
...
```bash
$ sudo {{v10X.cli.bin}} collections list
```
...
```bash
$ sudo {{v10X.cli.bin}} hub update
$ sudo {{v10X.cli.bin}} collections upgrade crowdsecurity/sshd
```
