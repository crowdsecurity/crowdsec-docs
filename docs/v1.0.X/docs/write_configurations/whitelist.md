...
The whitelist can be based on several criteria :
 ...
 - a list of {{v10X.expr.htmlname}} expressions : if any expression returns true, event is whitelisted
...
and reload {{v10X.crowdsec.name}} : `sudo systemctl reload crowdsec`
...
again, let's restart {{v10X.crowdsec.name}} !
...
After reloading {{v10X.crowdsec.name}}, and launching (again!) nikto :
