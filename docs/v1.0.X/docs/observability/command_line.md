```bash
sudo {{v10X.cli.name}} metrics
```

This command provides an overview of {{v10X.crowdsec.name}} statistics provided by [prometheus client](/Crowdsec/v1/observability/prometheus/). By default it assumes that the {{v10X.crowdsec.name}} is installed on the same machine.
