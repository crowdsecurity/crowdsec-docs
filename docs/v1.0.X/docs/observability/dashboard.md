...
The {{v10X.cli.name}} command `{{v10X.cli.bin}} dashboard setup` will use [docker](https://docs.docker.com/get-docker/) to install [metabase docker image](https://hub.docker.com/r/metabase/metabase/) and fetch our metabase template to have a configured and ready dashboard. 
...
```bash
sudo {{v10X.cli.bin}} dashboard setup
```
...
<details>
  <summary>{{v10X.cli.name}} dashboard setup</summary>
...
Now you can connect to your dashboard, sign-in with your saved credentials then click on {{v10X.crowdsec.Name}} Dashboard to get this:


Dashboard docker image can be managed by {{v10X.cli.name}} and docker cli also. Look at the {{v10X.cli.name}} help command using

```bash
sudo {{v10X.cli.bin}} dashboard -h
```
...
```bash
sudo {{v10X.cli.bin}} dashboard remove [-f]
```
Optional arguments:
...
```bash
sudo {{v10X.cli.bin}} dashboard stop
```
...
```bash
sudo {{v10X.cli.bin}} dashboard start
```
