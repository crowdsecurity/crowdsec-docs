...
{{v10X.crowdsec.Name}} runtime revolves around a few simple concepts :

 - It read logs (defined via {{v10X.ref.acquis}} config)
 - Those logs are parsed via {{v10X.ref.parsers}} and eventually enriched
 - Those normalized logs are then matched against the {{v10X.ref.scenarios}} that the user deployed
 - When a scenario is "triggered", {{v10X.crowdsec.Name}} generates an {{v10X.alert.Htmlname}} and eventually one or more associated {{v10X.decision.Htmlname}} :
 ...
 - Those information (the signal, the associated decisions) are then sent to crowdsec's {{v10X.lapi.htmlname}} and stored in the database

As you might have guessed by now, {{v10X.crowdsec.Name}} itself does the detection part and stores those decisions.
Then, {{v10X.bouncers.htmlname}} can "consume" those decisions (via the very same {{v10X.lapi.htmlname}}) and apply some actual remediation.

## Crowd sourced aspect

 [[References](https://crowdsecurity.github.io/api_doc/index.html?urls.primaryName=CAPI)]

Whenever the {{v10X.lapi.htmlname}} receives an alert with associated decisions, the meta information about the alert are shared with our central api :
...
To do so, bouncers are going to query the local API to know if there is an existing decisions against a given IP, range, username etc. [You can find a list of existing bouncers on the hub]({{v10X.bouncers.url}})
...
Acquistion configuration defines which streams of information {{v10X.crowdsec.name}} is going to process.
...
Stages concept is central to data parsing in {{v10X.crowdsec.name}}, as it allows to have various "steps" of parsing. All parsers belong to a given stage. While users can add or modify the stages order, the following stages exist :
...
A parser usually has a specific scope. For example, if you are using [nginx](https://nginx.org), you will probably want to use the `crowdsecurity/nginx-logs` which allows your {{v10X.crowdsec.name}} setup to parse nginx's access and error logs.

Parsers are organized into stages to allow pipelines and branching in parsing.

See the [{{v10X.hub.name}}]({{v10X.hub.url}}) to explore parsers, or see below some examples :
...
Enrichment is the action of adding extra context to an event based on the information we already have, so that better decision can later be taken. In most cases, you should be able to find the relevant enrichers on our {{v10X.hub.htmlname}}.
...
Once again, you should be able to find the ones you're looking for on the {{v10X.hub.htmlname}} !

## Scenarios

[[References](/Crowdsec/v1/references/scenarios/)]

Scenarios is the expression of a heuristic that allows you to qualify a specific event (usually an attack).It is a YAML file that describes a set of events characterizing a scenario. Scenarios in {{v10X.crowdsec.name}} gravitate around the [leaky bucket](https://en.wikipedia.org/wiki/Leaky_bucket) principle.
...
See the [{{v10X.hub.name}}]({{v10X.hub.url}}) to explore scenarios and their capabilities, or see below some examples :
...
As usual, those can be found on the {{v10X.hub.htmlname}} !
...
The `Alert` object is modified by post-overflows and {{v10X.profiles.htmlname}}.
...
`Decisions` are generated by Local API (LAPI) when an `Alert` is received, according to the existing {{v10X.profiles.htmlname}}