---
id: haproxy_spoa
title: "HAProxy WAF QuickStart (SPOA)"
sidebar_label: "QuickStart - HAProxy (SPOA)"
description: "Protect applications behind HAProxy with CrowdSec's open-source Web Application Firewall (WAF), using the SPOA remediation component to forward requests."
---
...
```bash
sudo mkdir -p /usr/local/etc/crowdsec/acquis.d
sudo tee /usr/local/etc/crowdsec/acquis.d/appsec.yaml > /dev/null <<'EOF'
appsec_configs:
  - crowdsecurity/appsec-default
labels:
  type: appsec
...
Edit the SPOA bouncer configuration and add the `appsec_url` plus the `appsec` block under your host(s):
...
```yaml title="/usr/local/etc/crowdsec/bouncers/crowdsec-spoa-bouncer.yaml"
# AppSec (WAF forwarding)
appsec_url: "http://127.0.0.1:7422"
appsec_timeout: "200ms"
...
```bash
sudo service crowdsec_spoa_bouncer restart
```
