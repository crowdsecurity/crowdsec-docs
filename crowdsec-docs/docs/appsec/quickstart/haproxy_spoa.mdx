...
On FreeBSD, use `/usr/local/etc/crowdsec/acquis.d/appsec.yaml` and reload the
rc.d service instead:
...
On FreeBSD, edit
`/usr/local/etc/crowdsec/bouncers/crowdsec-spoa-bouncer.yaml` and restart the
rc.d service:
