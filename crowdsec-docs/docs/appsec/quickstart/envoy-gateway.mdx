...
### 6. Use the upstream LAPI env var name in raw manifests

If you deploy the bouncer with raw manifests, use
`ENVOY_BOUNCER_BOUNCER_APIURL`, not `ENVOY_BOUNCER_BOUNCER_LAPIURL`. The
upstream `kdwils/envoy-proxy-bouncer` project uses `bouncer.apiURL` /
`ENVOY_BOUNCER_BOUNCER_APIURL` for CrowdSec LAPI connectivity.

### 7. `BOUNCER_KEY_envoy` may still need one explicit registration
...
### 8. An invalid `ENROLL_KEY` can break the whole rollout
