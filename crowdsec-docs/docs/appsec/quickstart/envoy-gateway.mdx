...
```yaml title="app-a-securitypolicy.yaml"
...
```yaml title="app-b-securitypolicy.yaml"
apiVersion: gateway.envoyproxy.io/v1alpha1
kind: SecurityPolicy
metadata:
  name: crowdsec-ext-auth
...
```bash
kubectl apply -f app-a-securitypolicy.yaml
kubectl apply -f app-b-securitypolicy.yaml
```
...
```bash
kubectl -n app-a describe securitypolicy crowdsec-ext-auth
kubectl -n app-b describe securitypolicy crowdsec-ext-auth
```
