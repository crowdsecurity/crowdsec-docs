...
### Monitor attacks in the Console

The **[CrowdSec Console](https://app.crowdsec.net)** provides a real-time visual dashboard of every WAF block — attack type, source IP, country, and the exact rule triggered. AppSec alerts are tagged with labels like `appsec:sqli` and `appsec:xss` for easy filtering.

If you haven't connected the Console yet, follow the [enrollment guide](/u/getting_started/post_installation/console). Once enrolled, WAF alerts populate automatically:

![AppSec alerts in the CrowdSec Console](/img/appsec_console.png)

### Upgrade your protection level

Want broader attack coverage? Upgrade from virtual patching to OWASP CRS detection in one command:

```bash
# Add out-of-band CRS (no false positives, behavioral banning)
sudo cscli collections install crowdsecurity/appsec-crs
```

Then update `/etc/crowdsec/acquis.d/appsec.yaml` to include `crowdsecurity/crs` and restart CrowdSec. See [WAF Deployment Strategies](/appsec/advanced_deployments.mdx) for all three protection profiles.

### Other next steps

 - [WAF Deployment Strategies](/appsec/advanced_deployments.mdx) — gradually increase protection from virtual patching to full OWASP CRS
 - [Rules syntax](/appsec/rules_syntax.md) and [creation process](/appsec/create_rules.md) — write your own rules and contribute to the Hub
 - [Performance benchmarks](/appsec/benchmark.md) — understand the latency impact
