...
:::warning
This is only for advance users that wish to compile their own software. If you are not comfortable with this, please use the [official packages](/getting_started/getting_started.md)
:::

We define systems by there underlying distribution rather than a fork or modification of a distribution. For example, Ubuntu and Debian are both Debian based distributions, so they will share the same instructions as the term DEB. Centos and Fedora are both Redhat based distributions, so they will share the same instructions as the term RPM. Arch is just Arch, so it will have its own instructions.
...
- `BUILD_VERSION=v1.2.3` - The version you want to build. This is required for the build to work.
...
- `BUILD_STATIC=1` - Build a static binary:
  - DEB: none
  - RPM/Arch:
    - RPM: Enable [crb repo](https://linux.how2shout.com/enable-crb-code-ready-builder-powertools-in-almalinux-9/#:~:text=CRB%20(Code%20Ready%20Builder)%20is,their%20workflow%20and%20increase%20productivity.)
...
```bash
make BUILD_STATIC=1 BUILD_VERSION=v1.2.3 release
```
...
```bash
make BUILD_VERSION=v1.2.3 build
```
