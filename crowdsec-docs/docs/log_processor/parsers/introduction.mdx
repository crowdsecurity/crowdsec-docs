...
For example if you have a `syslog` Data Source and a `container` Data Source writing the same application log lines you wouldn't want `s01-parse` to handle this logic twice, since `s00-raw` can normalize the logs into a predictable format. 
...
Currently we have a few enrichers available on the [Hub](https://hub.crowdsec.net/), that are installed by default so you don't need to worry about this stage unless you want to create your own.
