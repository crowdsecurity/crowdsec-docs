...
The event goes via various steps :
 - the `filter` decides event eligibility : if the expression is true, the event "enters" the bucket
 - the optional `groupby` expression segments buckets, typically by `source_ip`: this ensures each source IP has its own bucket and is accounted for properly
