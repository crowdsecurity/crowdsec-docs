...
```
sudo crowdsec-cloudflare-bouncer -c <path_to_config_file> -d 
```
...
```bash
sudo crowdsec-cloudflare-bouncer -c <path_to_config_file> -d
sudo crowdsec-cloudflare-bouncer -c <path_to_config_file> -g <TOKEN_1>,<TOKEN_2> -o <path_to_config_file>
sudo systemctl restart crowdsec-cloudflare-bouncer
```

:::note
You can omit `-c <path_to_config_file>` if you're using default config file.
It is located at `/etc/crowdsec/bouncers/crowdsec-cloudflare-bouncer.yaml`
:::
...
Valid choice includes either of either of 'block', 'js_challenge', 'challenge', 'managed_challenge' or 'none' to ignore unsupported decision.
