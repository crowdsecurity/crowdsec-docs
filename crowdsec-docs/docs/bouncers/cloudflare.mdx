...
only_include_decisions_from: # only include IPs banned due to decisions orginating from provided sources. eg value ["cscli", "crowdsec"]
    - CAPI # decisions from API community blocklist
    - cscli # decisions created via cscli decisions add ...
    - crowdsec # decisions created due to triggering of scenarios
    - lists # decisions obtained from list subscriptions
...
Valid choice includes either of either of 'block', 'js_challenge', 'challenge', 'managed_challenge'. 
...
```bash
crowdsec-cloudflare-bouncer -g <TOKEN_1>,<TOKEN_2>... > cfg.yaml
