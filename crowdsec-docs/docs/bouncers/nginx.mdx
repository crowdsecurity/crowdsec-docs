...
The best way to migrate from the crowdsec-nginx-bouncer v0.* to v1 is to reinstall the bouncer. Indeed, many news configurations options are now available and some has been removed.

- Backup your CrowdSec Local API key from your configuration file (`/etc/crowdsec/bouncers/crowdsec-nginx-bouncer.conf`)
- Remove the old bouncer:

```bash
sudo apt-get remove --purge crowdsec-nginx-bouncer
```
...
- Configure the bouncer in `/etc/crowdsec/bouncers/crowdsec-nginx-bouncer.conf`
