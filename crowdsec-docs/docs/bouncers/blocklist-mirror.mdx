...
```bash
git clone https://github.com/crowdsecurity/crowdsec-blocklist-mirror.git
cd crowdsec-blocklist-mirror/
make release
tar xzvf crowdsec-blocklist-mirror.tgz
...
crowdsec_config:
  ...
  only_include_decisions_from: ["crowdsec", "cscli", "CAPI", "lists"]
...
Format of the blocklist. Currently only `fortinet` is supported.
...
Currently only "basic" authentication is supported. You can disable it authentication completely by setting this to 'none'.
