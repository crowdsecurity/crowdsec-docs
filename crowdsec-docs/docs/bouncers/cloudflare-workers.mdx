...
:::note
By default the generated config only sets up challenge remediation for all the zones.
...
IP's originating from decisions with `captcha` action would be handled by captcha remediation and decisions with `ban` action would be handled by ban remediation.

Decisions with other actions would be applied remedications specified by `cloudflare_config.accounts[*].zones[*].default_action` value.
:::
