...
```bash
make build
./crowdsec-cloudflare-bouncer -g <CLOUDFLARE_TOKEN1>,<CLOUDFLARE_TOKEN2> > cfg.yaml # auto-generate cloudflare config for provided space separated tokens
vi cfg.yaml # review config and set `crowdsec_lapi_key`
./crowdsec-cloudflare-bouncer -c cfg.yaml # run the bouncer
```
...
Path to the TLS private key file used for [TLS authentication with CrowdSec LAPI](/docs/local_api/tls_auth).

### `cert_path`

Path to the TLS certificate file used for[TLS authentication with CrowdSec LAPI](/docs/local_api/tls_auth).

### `ca_cert_path`

Path to the TLS CA certificate file used for [TLS authentication with CrowdSec LAPI](/docs/local_api/tls_auth).
