...
`filter` must be a valid [expr](../expr) expression that will be evaluated against the {{v1X.event.htmlname}}.
...
This section is only relevant when `cscli` is used to install scenario from hub, as ill download the `source_url` and store it to `dest_file`. 
