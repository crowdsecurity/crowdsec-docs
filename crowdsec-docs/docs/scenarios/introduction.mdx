...
Scenarios receive [events](/concepts.md#events) and can produce [alerts](/concepts.md#alerts) using the [leaky bucket](https://en.wikipedia.org/wiki/Leaky_bucket.md) algorithm.
...
Once an overflow happens, it will go through [postoverflows](/parsers/introduction.mdx#postoverflows) to handle last chance whitelists, before being finally turned into a potential [decision](/docs/concepts#decisions) by [profiles](/profiles/intro.md).
