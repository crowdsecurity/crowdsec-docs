...
One of the advantages of CrowdSec when compared to other solutions is its crowd-sourced aspect: Meta information about detected attacks (Source IP address, time and triggered scenario) are sent to a central API and then shared amongst all users.
...
CrowdSec, besides the core "detect and react" mechanism, is committed to a few other key points :
 ...
 - **Reproducibility** : CrowdSec can run not only against live logs, but [as well against cold logs](/user_guides/replay_mode.md). It makes it a lot easier to detect potential false-positives, perform forensic or generate reporting
