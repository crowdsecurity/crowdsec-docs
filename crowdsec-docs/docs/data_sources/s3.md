...
How often in seconds crowdsec will check for new objets in a bucket when using the `list` polling method.
Defaults to 60.
...
Only read objets matching this prefix when `polling_method` is `list`.
