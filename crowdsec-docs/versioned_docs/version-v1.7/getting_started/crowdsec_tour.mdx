...
Check [decisions management](/u/user_guides/decisions_mgmt) for more!
...
There are different decisions `SOURCE`:
  ...
  - `cscli`: decisions added via `sudo cscli decisions add`
...
The indicators are grouped by scope :
 ...
 - Buckets: which buckets were created and/or overflowed (scenario efficiency)
 - Acquisition: which files produce logs and whether they are parsed (or end up in buckets)
...
The `Acquisition Metrics` table is a great way to check whether your parsers are set up correctly:
...
The `Parser Metrics` will let you troubleshoot eventual parser misconfigurations :
 ...
 - **HITS** is how many events were fed to this specific parser
...
The `cscli dashboard` command (Metabase) was removed in CrowdSec 1.7.0. To visualize alerts and decisions, use the [CrowdSec Console](https://app.crowdsec.net) or [Prometheus and Grafana](/observability/prometheus.md#exploitation-with-prometheus-server--grafana).
...
CrowdSec uses go-routines for parsing and enriching logs, pouring events into buckets and managing outputs.
...
Please keep in mind that thanks to the [http API](https://crowdsecurity.github.io/api_doc/lapi/), the log parsing workload can be split among several agents pushing to a single [LAPI](/local_api/intro.md).
