...
A **Remediation Component** *(aka **Bouncer**)* is enforcing **decisions** made by **CrowdSec Security Engine** based on detected malicious behaviors [\[See figure 1\]](#crowdsec-security-engine-diagram), or directly by the CrowdSec SaaS endpoint serving public, CrowdSec or user-made blocklists.
...
Here is an existing remediation component *(bouncer)* for Nginx and its Lua dependency.
It's one of the most complete bouncers, with AppSec capabilities and metrics. A good example to follow for your implementation.  
...
⚠️ **Your bouncer must always delete/clean its resources on shutdown**
...
* URL to Local API endpoint: configurable field **api_url**  
  * Default value likely to be: `http://127.0.0.1:8080`
  * Security Engine Config : /etc/crowdsec/config.yaml // api.server.listen_uri  
...
* Parameters  
  ...
  * **origins:** empty by default, meaning all origins are considered  
...
* Storing decisions  
  ...
  * The decisions format is the following:  
    ...
    * Store each decision independently, as each has its own remediation action and TTL  
...
* Remediation type  
  ...
  * **captcha**  
    * Various types of captcha must be supported  
    * configurable fields:  
      * captcha_provider  
      * captcha_secret_key  
      * captcha_site_key  
...
The metrics push interval should be configurable, with a default value of 30 minutes and not allow intervals smaller than 10 minutes. Setting the interval to 0 disables the push.
...
- The startup timestamp in UTC  
- A list of metrics, each entry holding:  
  - Meta information about the payload itself: the push interval in seconds, the push timestamp in UTC  
  - A list of items: each item must have a name, value, unit, and, optionally, one or more labels

The metrics track the number of dropped requests per decision origin, so the RC must track internally the origin of every decision (based on the `origin` field from the decision stream).  
Each push must reset the internal counter for the metrics (i.e., we have only sent the number of dropped requests since the last push).  
Each `dropped` metric must have an `origin` label whose value is the origin of the decision and a `remediation` label whose value is the type of remediation that was applied (e.g., `ban` or `captcha`).  
...
* Connect to AppSec endpoint  
  ...
  * Auth by API key passed in the header **X-Crowdsec-Appsec-Api-Key** (same value as the LAPI api_key)  
* Request forwarding  
...
Usually we (at CrowdSec) will deal with **documentation**, **install scripts** and **packaging**. Any pointers from the bouncer's developer that can help with these processes are welcome:
...
* The bouncer config file name pattern is the following:  *crowdsec-\<service\>-bouncer.yaml* (*.conf* for nginx, openresty, haproxy and apache2 bouncers)  
* Example of config file */etc/crowdsec/bouncers/crowdsec-apache2-bouncer.conf*
...
If you installed your CrowdSec on a server with internet access, and it’s able to communicate with our Central API, it will periodically retrieve the community blocklist. If your CrowdSec instance shares signals, you'll get up to 15k decisions from the community blocklist, or up to 60k on paid plans (decisions origin will be CAPI); if not, you’ll receive up to 3k.
...
* Via **cscli decisions import**.    
  ...
  * The origin of those decisions will be `cscli-import`
...
* AppSec [quickstart guide here](https://docs.crowdsec.net/docs/next/appsec/quickstart/general_setup/), [Testing example here](https://docs.crowdsec.net/docs/next/appsec/quickstart/general_setup/#testing-waf-component)  
...
* **scopes:** “ip,range” is the only relevant value when remediating on IPs  
* **origins:** Leave blank to allow all origins, check your configurable origins against the [decision example](#decision-example)  
...
* **scope:** “ip” is the only relevant value when remediating on IPs  
...
```json
{
  "remediation_components": [{
	...
	"feature_flags": [], //Always empty / invalid / ignored for bouncers
