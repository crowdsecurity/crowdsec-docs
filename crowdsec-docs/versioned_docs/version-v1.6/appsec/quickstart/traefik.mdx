...
:::info
Before starting, ensure you are using the [Traefik Plugin](https://plugins.traefik.io/plugins/6335346ca4caa9ddeffda116/crowdsec-bouncer-traefik-plugin) and **NOT** the older [traefik-crowdsec-bouncer](https://app.crowdsec.net/hub/author/fbonalair/remediation-components/traefik-crowdsec-bouncer), as it hasn't received updates to use the new AppSec Component.
:::
...
These steps change depending on how you run the Security Engine. If you are using `docker run`, launch the container from the same directory as `appsec.yaml`. If you are using `docker-compose`, use a relative file mount to mount `appsec.yaml`.

Steps:
    1. Change to the location where you executed the `docker run` or `docker compose` command.
    2. Create an `appsec.yaml` file at the base of the directory.
...
Follow the [Stack health check](/u/getting_started/health_check) to confirm the CrowdSec engine, AppSec Component, and Traefik bouncer are working together as expected.
