---
id: general_setup
title: "WAF General Setup"
sidebar_label: "General Setup"
description: "Core setup for the CrowdSec Web Application Firewall (WAF): install the rule collections, configure the AppSec acquisition, and point your bouncer at it."
---
...
- **CrowdSec Security Engine** (>= 1.6.0) installed and running
...
:::note
The output may look different depending on which command you used, but as long as you see the port and the process `crowdsec`, it means the AppSec Component is running.
:::
...
- Reviewing the [AppSec troubleshooting guide](/appsec/troubleshooting.md) if you need to investigate or refine the deployment.
- Exploring [WAF deployment strategies](/appsec/advanced_deployments.mdx) when you are ready to expand beyond the initial setup.

### Testing Detection

If you've enabled a WAF-capable bouncer, you can trigger the `crowdsecurity/appsec-generic-test` dummy scenario.
This scenario does not lead to a decision, but it is a good way to confirm the setup is working.
