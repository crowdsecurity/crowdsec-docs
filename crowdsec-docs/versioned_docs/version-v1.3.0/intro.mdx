...
CrowdSec, besides the core "detect and react" mechanism,  is committed to a few other key points :
 ...
 - **Reproducibility** : Crowdsec can run not only against live logs, but [as well against cold logs](/u/user_guides/replay_mode). It makes it a lot easier to detect potential false-positives, perform forensic or generate reporting
