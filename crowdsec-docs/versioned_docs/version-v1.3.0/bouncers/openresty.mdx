...
You need to add `include /usr/local/openresty/nginx/conf/conf.d/*.conf;` in nginx config file `/usr/local/openresty/nginx/conf/nginx.conf` at the **server** section.
...
To make HTTP request in the bouncer, we need to set a `resolver` in the configuration. We choose `8.8.8.8` since we query `google.com` for the captcha verification, but you can replace it with a valid one.
...
If you want to use reCAPTCHA with your OpenResty Bouncer, you must provide a reCAPTCHA Site key and Secret key in your bouncer configuration.
...
### Why remediation are not apply instantaneous
...
The best way to migrate from the crowdsec-openresty-bouncer v0.* to v1 is to reinstall the bouncer. Indeed, many news configurations options are now available and some has been removed.
...
The fallback remediation if the bouncer receive a decision with an unknown remediation.
...
Timeout in second for the HTTP requests done by the bouncer to query CrowdSec local API or www.google.com (for the reCAPTCHA verification).
...
The locations to exclude the boucing. It is a list of location, separated by commas.
