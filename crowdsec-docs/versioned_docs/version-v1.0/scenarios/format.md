...
`filter` must be a valid [expr](/docs/v1.0/expr/helpers) expression that will be evaluated against the event.
...
An [expression](/docs/v1.0/expr/helpers) that must return a string. This string will be used as a partition for the buckets.
...
An [expression](/docs/v1.0/expr/helpers) that must return a string. The event will be poured **only** if the string is not already present in the bucket.
...
`overflow_filter` is an [expression](/docs/v1.0/expr/helpers) that is run when the bucket overflows.
