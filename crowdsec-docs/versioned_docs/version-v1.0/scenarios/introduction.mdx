...
Scenarios receive [events](/docs/v1.0/concepts#events) and can produce [alerts](/docs/v1.0/concepts#alerts) using the [leaky bucket](https://en.wikipedia.org/wiki/Leaky_bucket) algorithm.
...
Once an overflow happens, it will go through [postoverflows](/docs/v1.0/parsers/intro#postoverflows) to handle last chance whitelists, before being finally turned into a potential [decision](/docs/v1.0/concepts#decisions) by [profiles](/docs/v1.0/profiles/intro).
