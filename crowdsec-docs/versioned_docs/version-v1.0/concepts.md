...
CrowdSec runtime revolves around a few simple concepts :
 ...
 - Those information (the signal, the associated decisions) are then sent to crowdsec's [Local API](/docs/v1.0/local_api/intro) and stored in the database

As you might have guessed by now, CrowdSec itself does the detection part and stores those decisions.
Then, [bouncers](user_guides/bouncers_configuration.md) can "consume" those decisions (via the very same [Local API](/docs/v1.0/local_api/intro) and apply some actual remediation.
...
[[References](/docs/v1.0/bouncers/intro)]
...
[[References](/docs/v1.0/parsers/intro#stages)]
...
[[References](/docs/v1.0/parsers/intro)]
...
You can as well [write your own](/docs/v1.0/parsers/create) !
...
[[References](/docs/v1.0/scenarios/format)]
...
You can as well [write your own](/docs/v1.0/scenarios/create) !


## Collections

[[References](/docs/v1.0/collections/intro)]
...
[[References](/docs/v1.0/expr/event)]
...
The `Event` object is modified by parses, scenarios, and directly via user [statics expressions](/docs/v1.0/parsers/format#statics) (for example).
...
[[References](/docs/v1.0/expr/alert)]

An `Alert` is the runtime representation of a bucket overflow being processed by crowdsec : It is embedded in an Event.

The `Alert` object is modified by post-overflows and [profiles](/docs/v1.0/profiles/intro).

## Decisions

[[References](/docs/v1.0/expr/decision)]
...
`Decisions` are generated by Local API (LAPI) when an `Alert` is received, according to the existing [profiles](/docs/v1.0/profiles/intro)