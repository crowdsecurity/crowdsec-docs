...
CrowdSec, besides the core "detect and react" mechanism,  is committed to a few other key points :
 ...
 - **Easy daily operations** : Using [cscli](/docs/v1.0/about_docs/documentation_about) and the [hub](http://hub.crowdsec.net), keeping your detection mechanisms up-to-date is trivial
 - **Reproducibility** : Crowdsec can run not only against live logs, but [as well against cold logs](/docs/v1.0/user_guides/replay_mode). It makes it a lot easier to detect potential false-positives, perform forensic ou generate reporting
 - **Observability** : Providing strongs insights on what is going on and what crowdsec-agent is doing :
  ...
  - **API Centric** : All the components are comunicating via an [HTTP API](/docs/v1.0/local_api/intro), allowing easy multi-machines setups
