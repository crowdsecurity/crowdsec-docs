...
CrowdSec, besides the core "detect and react" mechanism,  is committed to a few other key points :
 ...
 - **Easy Installation** : [Out of the box](/docs/getting_started/install_crowdsec) on most standard setupts
 - **Easy daily operations** : Using [cscli](/docs/about_docs/documentation_about) and the [hub](http://hub.crowdsec.net), keeping your detection mechanisms up-to-date is trivial
 - **Reproducibility** : Crowdsec can run not only against live logs, but [as well against cold logs](/docs/user_guides/replay_mode). It makes it a lot easier to detect potential false-positives, perform forensic ou generate reporting
 - **Observability** : Providing strongs insights on what is going on and what crowdsec-agent is doing :
    - Humans have [access to a trivially deployable web interface](/docs/observability/dashboard)
    - OPs have [access to detailed prometheus metrics](/docs/observability/prometheus)
    - Admins have [a friendly command-line interface tool](/docs/observability/cscli)
  - **API Centric** : All the components are comunicating via an [HTTP API](/docs/local_api/intro), allowing easy multi-machines setups
