...
For those that prefer hands-on approach, you can as well [manually install crowdsec](/docs/user_guides/building).

## Install our repositories

Installing our repositories allows you to access the latest packages of crowdsec and bouncers.
We are using [packagecloud.io service](https://packagecloud.io/crowdsec/crowdsec/), for those that prefer, manual repositories installation method is available [here](https://packagecloud.io/crowdsec/crowdsec/install#manual).
...
You now have crowdsec running ! You can move forward and install a [bouncer]({{TBD}}), or take a [tour]({{TBD}}) of the software beforehand !


:::caution

Keep in mind that CrowdSec is only in charge of the "detection", and won't block anything on its own.
You need to deploy a [bouncer](../bouncers/) to "apply" decisions.
