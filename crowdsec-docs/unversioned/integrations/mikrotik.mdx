...
The MikroTik format emits a full command per entry, so entries are large. When pulling **without compression**, keep `page_size` at or below **~35,000** entries to stay under the ~5 MB response limit; beyond that the response is truncated. Enabling compression avoids this limit entirely.
