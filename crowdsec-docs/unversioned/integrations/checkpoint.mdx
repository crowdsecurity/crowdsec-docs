...
## Create a Checkpoint Integration Endpoint
...
Select the Check Point feed format (if you use a Raw IP List integration instead, select a custom CSV format and set the data column to `1`), then click **Test Feed**.
...
The Checkpoint format is verbose. When pulling **without compression**, keep `page_size` at or below **~60,000** entries to stay under the ~5 MB response limit; larger pages return an error. Enabling compression avoids this limit entirely.
