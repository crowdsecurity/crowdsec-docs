...
```
 ...
 - [Redoc method link](https://admin.api.crowdsec.net/v1/docs#tag/Blocklists/operation/getBlocklist)
...
Subscribing makes the target pull and enforce the list. Choose who to target with `entity_type`:
the **whole org** (no `ids`), **specific engines**, or **a set of tags**. Engine and tag ids come
from the Console — see the
[subscription mechanism](/console/service_api/blocklists.md#blocklist-subscription-mechanism) for
details and the `{ "updated": [...], "errors": [...] }` response.
...
curl -i -H "x-api-key: ${KEY}" -X POST -H "Content-Type: application/json" \
https://admin.api.crowdsec.net/v1/blocklists/1234MYBLOCKLISTID/subscribers \
-d '{ "entity_type": "tag", "ids": ["env:prod"], "remediation": "ban" }'
```
