...
parameters, which filter server-side:
...
Inside the response, each metric's `data` is grouped by **origin** — a string such as `CAPI`,
`crowdsec`, `ipset`, `unknown`, or a blocklist object (with its `label`/`id`) — **not** per engine.
To get a single engine's numbers, request with that engine's `engine_ids`; a single response never
splits the totals per engine.
:::
