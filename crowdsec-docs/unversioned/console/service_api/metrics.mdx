...
```
 ...
 - [Redoc method link](https://admin.api.crowdsec.net/v1/docs#tag/Metrics/operation/getMetricsRemediation)
...
By default the metrics cover your whole organization. Narrow them with these repeatable query
parameters; they filter server-side:
...
:::note The breakdown is by origin, not by engine
...
- `dropped`: The traffic that was discarded by your remediation components (bouncers) and Blocklists. It represents the data received from the different remediation components, so it can be:
...
- `processed`: The traffic that was processed by your remediation components (bouncers) and Blocklists. Its represent the data received from differents remediation components, so it can be :
...
The computed metrics are estimations based on the raw metrics. They are calculated by applying a coefficient to remediation components (bouncers) metrics that approximates the saved resources.
...
- `saved`:
    ...
    - `egress_traffic`: The amount of egress traffic that was saved. It is the amount of egress traffic that would have been spent to serve responses if the requests were not dropped.
