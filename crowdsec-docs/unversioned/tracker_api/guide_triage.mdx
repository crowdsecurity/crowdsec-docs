...
- **Active Exploitation**: The threat is active and real. Treat as urgent.
- **Background Noise**: Ongoing but low-level. Standard patching is appropriate.
- **Limited Exploitation**: Limited attacker interest. Low urgency.
- **Insufficient Data**: Not enough telemetry to judge; fall back to CVSS and vendor advisories.
...
| High CrowdSec Score + Active exploitation | **Urgent patch.** Deploy blocklist. Check all instances of affected software. |
| Moderate score + Background noise | **Prioritize patch** within days, not weeks. Blocklist optional. |
| Low score + Limited Exploitation | **Standard patch** cycle. Monitor for phase changes. |
...
2. If not, **create a firewall integration** (if you don't have one) — see [Integrations & Blocklists](./api_integrations.mdx)
...
def triage_cve(cve_id: str) -> str:
    ...
    if score >= 7 or phase in ("mass_exploitation",):
