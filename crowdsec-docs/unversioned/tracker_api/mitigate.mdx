...
integrations_service = Integrations(base_url=Server.production_server.value, auth=auth)
...
auth = ApiKeyAuth(api_key=KEY)
cves_service = Cves(base_url=Server.production_server.value, auth=auth)
