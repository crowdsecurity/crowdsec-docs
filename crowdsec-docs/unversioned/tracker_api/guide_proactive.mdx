...
1. Create a firewall integration (if you don't have one) — see [Integrations & Blocklists](./api_integrations.mdx)
...
For threats outside your subscribed vendors, you can also subscribe to individual [fingerprint rules](./fingerprints_vs_cves.mdx) or CVEs:
...
# Get trending CVEs
response = cves_service.get_cves(page=1, size=50, detailed=True)
