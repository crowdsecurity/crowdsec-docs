...
It exposes the same data as the [API](./api_authentication.mdx) in a visual format designed for SOC analysts and security managers.
...
Each threat card shows the exploitation phase as a colored badge (e.g., "ACTIVE EXPLOITATION", "BACKGROUND NOISE"), the CrowdSec Score, and a miniature activity chart so you can quickly spot trends.
...
Displays all Reconnaissance rules — detection patterns for product-level probing activity. See [Fingerprints vs CVEs](./fingerprints_vs_cves.mdx) for the difference between CVEs and Reconnaissance rules.
...
- **Phase badge**: The current exploitation phase as a colored label (e.g., "ACTIVE EXPLOITATION")
...
See [Scores & Ratings](./scores.mdx) for detailed interpretation of these values.
...
See [CrowdSec Analysis](./crowdsec_analysis.mdx) for how to use this in triage and reporting.
