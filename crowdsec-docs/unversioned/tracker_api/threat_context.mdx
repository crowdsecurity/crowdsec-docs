...
The `threat_context` object provides geographic and industry-level intelligence about exploitation activity for a given CVE or reconnaissance rule. While [scores](./scores.mdx) tell you *how urgent* a threat is and [exploitation phases](./exploitation_phases.mdx) tell you *where in the lifecycle* it sits, threat context answers a different set of questions:
...
Consider CVE-2024-0012 (PAN-OS Authentication Bypass). Its threat context shows:
...
- **API**: Returned in the `threat_context` field of the [CVE endpoints](./api_cves.mdx) (`/v1/cves` and `/v1/cves/{cve_id}`) and [Fingerprint endpoints](./api_fingerprints.mdx) (`/v1/fingerprints` and `/v1/fingerprints/{fingerprint}`).
