...
|-----------|------------|
| **recency** | A bonus applied when exploitation activity is very recent. A CVE last seen exploited today gets a recency boost compared to one last seen a month ago, all else being equal. |
| **total** | The net adjustment applied to the score. |

These adjustments are surfaced so you can understand *why* a score is what it is. For example, if a CVE has a CrowdSec Score of 6 with an adjustment_score.recency of +1, you know that 1 point of that score comes from very recent activity rather than sustained long-term exploitation.
