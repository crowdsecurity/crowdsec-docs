...
```
  - **solution** regenerate the API key via [cscli bouncers](/docs/next/cscli/cscli_bouncers_add) and replace the old one in the bouncer configuration file. Do not reuse the same key name.
