...
:::info
This command should **ONLY** be run on the parent node.
:::


### How do I know if my setup is working correctly? Are some unparsed logs normal?

Yes, Security Engine parsers only parse the logs that are relevant for scenarios.

Take a look at `cscli metrics` [and understand what they mean](/docs/next/observability/cscli) to know if your setup is correct.
...
### Why are some logs not parsed in `cscli metrics`?

If logs do not seem to be parsed correctly, use [`cscli explain`](/docs/cscli/cscli_explain):
...
This command shows how each parser behaves.
...
### I want to add collection X: how do I add log files and test that it works?
...
Additional filtering is possible; see [cscli decisions list](/docs/next/cscli/cscli_decisions_list) for more information.
...
Recommended ways to prevent a specific IP from being banned:
...
It can discard events based on a specific URL, source IP address, or any other condition.
...
- Are sent to Central API and appear in the Console by default (unless `cscli console disable tainted` has been specified)

"local" items:

- Have been locally created by the user
- Are not managed by `cscli` operations
- Are sent to Central API and appear in the Console by default (unless `cscli console disable custom` has been specified)
...
For example, you have a scenario called `crowdsecurity/my-custom-scenario` located in `{config_dir}/scenarios/my_custom_scenario.yaml`.
...
Please review your acquisition files and remove any duplicate log entries.
...
CrowdSec stores its data in `/var/lib/crowdsec/data/` on Linux, `/var/db/crowdsec/data/` on FreeBSD (including OPNsense and pfSense), and `C:\ProgramData\CrowdSec\data\` on Windows.
...
Security Engine supports SQLite (default), MySQL, and PostgreSQL databases.
...
For multi-server setups, use one of the following:
...
These are the default log locations. Third-party integrations may use different paths.
...
```
:::info
There may be other variations of this message. If it contains `connection refused`, `connection reset by peer`, or `no such host`, it is likely a connectivity or configuration issue.
:::
...
To report a bug, open an issue on the affected component repository:
...
The Security Engine and Remediation Components are provided under the [MIT license](https://en.wikipedia.org/wiki/MIT_License).

### What is CrowdSec Security Engine?

CrowdSec Security Engine is open-source security software. See the [overview](/docs/next/intro).

### What language is it written in?

CrowdSec Security Engine is written in [Golang](https://go.dev/).

### What resources are needed to run Security Engine?

Security Engine itself is rather light, and in a small to medium setup should use less than 100 MB of memory.
...
If you need help with large-scale deployment, please contact us through this [form](https://www.crowdsec.net/contact-crowdsec).
...
Setting up a proxy works out of the box, the [net/http golang library](https://go.dev/src/net/http/transport.go) can handle those environment variables:
...
All HTTP requests made to the Central API will then go through the [Tor network](https://www.torproject.org/).
