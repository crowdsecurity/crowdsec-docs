...
description: "Protect a PHP application without a reverse proxy: a standalone bouncer that checks visitors against CrowdSec decisions and the AppSec (WAF) engine."
sidebar_position: 1
---
...
<BouncerHeader
  ...
  githubUrl="https://github.com/crowdsecurity/cs-standalone-php-bouncer"
  hubUrl="https://app.crowdsec.net/hub/author/crowdsecurity/remediation-components/cs-php-bouncer"
/>
...
To obtain an API key, you can run the `cscli bouncers add` command:
...
To use the stream mode, you first have to set the `stream_mode` setting value to `true` in your `scripts/settings.php` file.
...
- Remove the `auto_prepend_file` directive that points to the `bounce.php` file  and restart your web server
...
You should see a list of tags (`vX.Y.Z` format) that have been published since your initial installation.
...
- Cap remediation level (e.g. for sensitive websites, ban will be capped to captcha)
...
- `auth_type`: Select from `api_key` and `tls`. Choose if you want to use an API key or TLS (PKI) authentication.
  TLS authentication is only available if you use CrowdSec agent with a version superior to 1.4.0.


- `api_key`: Key generated by the cscli (CrowdSec cli) command like `cscli bouncers add standalone-bouncer`.
...
- `use_curl`: By default, this lib calls the REST Local API using the `file_get_contents` method (`allow_url_fopen` is required).
  You can set `use_curl` to `true` in order to use `cURL` request instead (`ext-curl` is then required)
...
- `geolocation`: Settings for geolocation remediation (i.e. country based remediation).
  ...
  - `geolocation[cache_duration]`: This setting will be used to set the lifetime (in seconds) of a cached country
    associated to an IP. The purpose is to avoid multiple calls to the geolocation system (e.g. MaxMind database). Default to 86400. Set 0 to disable caching.
...
The remediation expects decisions with a scope of `Country` and a 2-letter country code as value.
