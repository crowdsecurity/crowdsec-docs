...
This remediation allows you to protect your PHP application from IPs that have been detected by CrowdSec. Depending on
...
To be able to use this remediation, the first step is to install [CrowdSec v1](https://doc.crowdsec.net/docs/getting_started/install_crowdsec/). CrowdSec is only in charge of the "detection", and won't block anything on its own. You need to deploy a bouncer to "apply" decisions.

Please note that first and foremost a CrowdSec agent must be installed on a server that is accessible by this remediation.

### Server and remediation setup

Once you set up your server as below, every browser access to a PHP script will be bounced by the standalone remediation.
...
- Optionally, if you want to use the standalone remediation in stream mode, you will have to set a cron task to refresh
  cache periodically.

#### Remediation sources copy
...
- Retrieve the last version of the remediation:

```bash
composer create-project crowdsec/standalone-bouncer /var/www/crowdsec-standalone-bouncer --keep-vcs
```

Note that we have to keep the vcs data as we will use it to update the remediation when a new version is available.
...
For a quick start, simply search for `YOUR_BOUNCER_API_KEY` in the `settings.php` file and set the remediation key.
To obtain a remediation key, you can run the `cscli` remediation creation command:
...
When a new release of the remediation is available, you may want to update sources to the last version.
...
Below are the steps to take to upgrade your current remediation:
...
- Built-in support for the most known cache systems Redis, Memcached and PhpFiles
  - Clear, prune and refresh the remediation cache
- Cap remediation level (ex: for sensitives websites: ban will be capped to captcha)

### Ban and captcha walls

When a user is suspected by CrowdSec to be malevolent, the remediation would either display a captcha to resolve or
...
- `tls_cert_path`: absolute path to the remediation certificate (e.g. pem file).
...
- `tls_key_path`: Absolute path to the remediation key (e.g. pem file).
  Only required if you choose `tls` as `auth_type`.
  **Make sure this path is not publicly accessible.** [See security note below](#security-note).
