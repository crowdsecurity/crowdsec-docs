...
- HTTP processing is driven by HAProxy `spoe-groups` and `http-request send-spoe-group`.
- Captcha is stateless (JWT-based) and requires a per-host `captcha.signing_key` (minimum 32 bytes).
- AppSec forwarding (WAF evaluation) is supported via `appsec_url` / `hosts[].appsec`.
- If you forward bodies (AppSec / captcha), enable `option http-buffer-request` and set `tune.bufsize 65536` (max).
- The captcha “validated” redirect uses `%[url]`.

Release notes: https://github.com/crowdsecurity/cs-haproxy-spoa-bouncer/releases/tag/v0.3.0-rc1

### HAProxy triggers SPOE groups

```haproxy
http-request send-spoe-group crowdsec crowdsec-http-body
...
If you run multiple SPOA instances serving the same domains, use the same `signing_key` everywhere so captcha tokens validate consistently.
...
Key used to sign captcha tokens (required when using captcha).
