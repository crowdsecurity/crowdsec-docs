...
description: "Wire HAProxy to CrowdSec over SPOA: the SPOA agent checks each connection against live decisions and hands HTTP requests to the AppSec (WAF) engine."
---
...
<BouncerHeader
  ...
  description="Go-based SPOA bouncer for HAProxy with WAF (AppSec) support — the recommended HAProxy integration."
  featuresSupport="mtls, streammode, metrics, prometheus, waf, captcha"
  githubUrl="https://github.com/crowdsecurity/cs-haproxy-spoa-bouncer"
  hubUrl="https://app.crowdsec.net/hub/author/crowdsecurity/remediation-components/cs-haproxy-spoa-bouncer"
/>
...
<Tabs
  ...
  values={[
    ...
    { label: 'FreeBSD', value: 'haproxy_freebsd' ,},
  ]
}>
...
<TabItem value="haproxy_freebsd">

```bash
sudo pkg install crowdsec haproxy crowdsec-haproxy-spoa-bouncer
```

The FreeBSD package installs an rc.d service named `crowdsec_spoa_bouncer`.
...
Use the tab for your operating system before applying the configuration snippets
below.

<Tabs
  groupId="haproxy-spoa-os"
  defaultValue="linux"
  values={[
...
If you are using packages, you will find the HAProxy configuration snippets in
`/usr/share/doc/crowdsec-haproxy-spoa-bouncer/examples`.

The examples below use these Linux package paths and services:

| Item | Linux package path or service |
| ---- | ----------------------------- |
| Bouncer config | `/etc/crowdsec/bouncers/crowdsec-spoa-bouncer.yaml` |
| SPOE config | `/etc/haproxy/crowdsec.cfg` |
| Main HAProxy config | `/etc/haproxy/haproxy.cfg` |
| Example configs | `/usr/share/doc/crowdsec-haproxy-spoa-bouncer/examples` |
| HTML templates for `lf-file` | `/var/lib/crowdsec-haproxy-spoa-bouncer/html` |
| Bouncer service | `crowdsec-spoa-bouncer` |

</TabItem>
<TabItem value="freebsd">

FreeBSD packages use the `/usr/local` prefix and rc.d services.

| Item | FreeBSD package path or service |
| ---- | ------------------------------- |
| Bouncer config | `/usr/local/etc/crowdsec/bouncers/crowdsec-spoa-bouncer.yaml` |
| SPOE config | `/usr/local/etc/haproxy/crowdsec.cfg` |
| Main HAProxy config | `/usr/local/etc/haproxy.conf` |
| Example configs | `/usr/local/share/doc/crowdsec-haproxy-spoa-bouncer/examples` |
| HTML templates for `lf-file` | `/usr/local/share/crowdsec-haproxy-spoa-bouncer/html` |
| Bouncer service | `crowdsec_spoa_bouncer` |

Enable the services:

```bash
sudo sysrc crowdsec_enable=YES
sudo sysrc crowdsec_spoa_bouncer_enable=YES
sudo sysrc haproxy_enable=YES
```

If the HAProxy rc.d script does not daemonize the process in your environment,
set explicit flags:

```bash
sudo sysrc haproxy_flags="-D -q -f /usr/local/etc/haproxy.conf -p /var/run/haproxy.pid"
```

Copy the packaged SPOE example into the FreeBSD HAProxy configuration directory:

```bash
sudo install -d -m 0755 /usr/local/etc/haproxy
sudo cp /usr/local/share/doc/crowdsec-haproxy-spoa-bouncer/examples/crowdsec.cfg \
  /usr/local/etc/haproxy/crowdsec.cfg
```

When using the non-Lua `lf-file` remediation block on FreeBSD, point HAProxy at
the FreeBSD template directory:
...
sudo service crowdsec_spoa_bouncer start
sudo service haproxy configtest
sudo service haproxy start

sockstat -4l | egrep '(:80|:8080|:9000)'
...
HAProxy 2.2+ can serve ban and captcha pages natively using `http-request return` with `lf-file` (log-format file). No Lua is required.
...
frontend http-in
    ...
    # Body size limit: stay safely under SPOE frame limit (~50 KB)
...
The complete example (with `global` and `defaults`) is also available at
`/usr/share/doc/crowdsec-haproxy-spoa-bouncer/examples/haproxy.cfg`.

</details>

### Optional: Lua-based template rendering

Lua support is **optional**. If you need dynamic template rendering beyond what `lf-file` provides (for example, injecting runtime variables not available as HAProxy log-format fetch methods), you can load the bundled Lua scripts instead.

Add to the `global` section of `haproxy.cfg`:
...
global
    ...
    setenv CROWDSEC_BAN_TEMPLATE_PATH /var/lib/crowdsec-haproxy-spoa-bouncer/html/lua/ban.html
    setenv CROWDSEC_CAPTCHA_TEMPLATE_PATH /var/lib/crowdsec-haproxy-spoa-bouncer/html/lua/captcha.html
```

Then replace the `http-request return` block in the frontend with:
...
The Lua example config is at `/usr/share/doc/crowdsec-haproxy-spoa-bouncer/examples/haproxy-lua.cfg`.
...
- Bot detection: route challenge responses and internal challenge assets
...
- **hCaptcha** — register at https://www.hcaptcha.com
- **reCAPTCHA** — register at https://www.google.com/recaptcha/admin (use **reCAPTCHA v2 "I'm not a robot"**)
- **Turnstile** — register at https://dash.cloudflare.com/?to=/:account/turnstile
  :::

### AppSec (WAF) forwarding — configuration reference

:::tip Turning this on
To enable the WAF end-to-end, follow the [AppSec Quickstart for HAProxy](/docs/next/appsec/quickstart/haproxy_spoa) — it picks up after the bouncer install and walks through collections, acquisition, and enabling forwarding.
:::

HAProxy requirements when using AppSec (and/or captcha):

- Increase HAProxy buffer size (max 64KB): `tune.bufsize 65536` (see in the example below)
- Enable request buffering: `option http-buffer-request` (see in the example below)
...
`tune.bufsize` is a global setting. `option http-buffer-request` must be set in
`defaults` or in each HTTP frontend that forwards requests to AppSec; it is not
a `global` option.
...
Because request-body forwarding is constrained by HAProxy/SPOE/SPOA limits, keep an explicit body size limit (for example `51200`) and consider a layered approach (IP remediation at HAProxy, deeper inspection downstream).

### Bot detection challenge

:::tip Prerequisites
Bot detection is an AppSec feature. First enable AppSec forwarding as described above, then follow the [bot detection guide](/docs/next/appsec/bot_detection/intro).
:::

Install the AppSec collections used by bot detection:

```bash
sudo cscli collections install crowdsecurity/appsec-virtual-patching crowdsecurity/appsec-generic-rules crowdsecurity/appsec-bot-challenge
```

Then configure an AppSec acquisition, for example in
...
`crowdsecurity/appsec-default` already loads the virtual patching and generic
rule families. Do not also list `crowdsecurity/virtual-patching` and
`crowdsecurity/generic-rules` in the same acquisition, otherwise rules may be
loaded twice.

HAProxy cannot receive the full bot-detection challenge page through the SPOE
frame. The SPOA bouncer therefore exposes a small HTTP listener for challenge
traffic, controlled by `challenge_http_listen`. The initial page is cached by
the bouncer, while the browser fetches its JavaScript assets and submits proof
through follow-up requests on the internal challenge paths:
...
When AppSec returns the `challenge` action, HAProxy must rewrite the request to
the bouncer-provided challenge URL and route it to that listener. The same
backend must also receive the internal challenge paths used by the browser:
`/crowdsec-internal/challenge/*`.

The request flow is:

1. AppSec returns the `challenge` action through SPOA and the bouncer stores the
   generated challenge under `/crowdsec-challenge/<token>`.
2. HAProxy rewrites the original request to that URL and sends it to the
   challenge HTTP listener. The client receives the challenge page with an HTTP
   `200` response, not the original AppSec `403` response.
3. The bouncer sets the `__crowdsec_challenge_relay` cookie. It is `HttpOnly`,
   limited to `/crowdsec-internal/challenge/`, and expires after five minutes.
4. The browser uses that cookie on the JavaScript and proof requests. HAProxy
   routes those requests to the same listener and passes the source address in
   `X-Crowdsec-Real-Src`. Missing, malformed, or expired relay cookies are
   rejected with `404`.

If you installed the bouncer from packages, start from the packaged examples in
`/usr/share/doc/crowdsec-haproxy-spoa-bouncer/examples/`. In particular,
`haproxy.cfg.gz` contains the challenge-enabled HAProxy example and
`crowdsec-spoa-bouncer.yaml.challenge` contains the bouncer keys above.

For an existing HAProxy configuration, add the challenge ACL, path rewrite,
routing rule, trusted source header, and challenge backend alongside your
regular SPOE configuration:

<details>
<summary>Challenge-enabled <code>/etc/haproxy/haproxy.cfg</code></summary>
...
frontend http-in
    ...
    acl crowdsec_challenge_backend_path path_beg /crowdsec-internal/challenge/
    ...
    # Required for challenge follow-up requests: this path bypasses SPOE, so the
    # bouncer needs HAProxy to pass the real client IP explicitly.
...
backend app
    mode http
    # Replace the placeholder with your real application backend before use.
    # Do not point this at CrowdSec LAPI (127.0.0.1:8080).
    server app1 <your-app-host>:<your-app-port>

backend crowdsec-spoa
    mode tcp
    timeout connect 2s
    timeout server 60s
    server spoa1 127.0.0.1:9000
...
backend crowdsec-challenge
    ...
    timeout connect 2s
    timeout server 60s
    server challenge1 127.0.0.1:9100
```

</details>

You can also add a small rate limit on the internal challenge paths so asset and
submission requests cannot bypass your normal application rate limits:
...
:::warning Preserve the trusted source IP
Always overwrite `X-Crowdsec-Real-Src` from HAProxy with `%[src]`; do not trust a
client-supplied value. If HAProxy is behind a CDN or another trusted proxy, set
the source IP with `http-request set-src ...` before the SPOE and challenge
routing rules.
:::

Verify the plumbing through HAProxy, not by calling the AppSec listener directly:

```bash
sudo crowdsec -t -c /etc/crowdsec/config.yaml
sudo crowdsec-spoa-bouncer -c /etc/crowdsec/bouncers/crowdsec-spoa-bouncer.yaml -t
sudo haproxy -c -f /etc/haproxy/haproxy.cfg
curl -i -H 'Accept: text/html' http://<your-haproxy-url>/
sudo cscli metrics show appsec
```

The response should be the CrowdSec challenge page, and the metrics should show
bot-detection challenge requests.
