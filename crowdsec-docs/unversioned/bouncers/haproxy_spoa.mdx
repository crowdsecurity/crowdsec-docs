...
description: "Wire HAProxy to CrowdSec over SPOP: the SPOA agent checks each connection against live decisions and hands HTTP requests to the AppSec (WAF) engine."
---
...
frame: the challenge response is large and spans follow-up requests for
JavaScript assets and submissions. The SPOA bouncer therefore exposes a small
HTTP listener for challenge traffic, controlled by `challenge_http_listen`:
