...
### FreeBSD paths and service names

FreeBSD packages use the `/usr/local` prefix and rc.d services. Replace the
Linux paths and commands in the examples as follows:

| Item | Linux package path or service | FreeBSD package path or service |
| ---- | ----------------------------- | ------------------------------- |
| Bouncer config | `/etc/crowdsec/bouncers/crowdsec-spoa-bouncer.yaml` | `/usr/local/etc/crowdsec/bouncers/crowdsec-spoa-bouncer.yaml` |
| SPOE config | `/etc/haproxy/crowdsec.cfg` | `/usr/local/etc/haproxy/crowdsec.cfg` |
| Main HAProxy config | `/etc/haproxy/haproxy.cfg` | `/usr/local/etc/haproxy.conf` |
| Example configs | `/usr/share/doc/crowdsec-haproxy-spoa-bouncer/examples` | `/usr/local/share/doc/crowdsec-haproxy-spoa-bouncer/examples` |
| HTML templates for `lf-file` | `/var/lib/crowdsec-haproxy-spoa-bouncer/html` | `/usr/local/share/crowdsec-haproxy-spoa-bouncer/html` |
| Bouncer service | `crowdsec-spoa-bouncer` | `crowdsec_spoa_bouncer` |
