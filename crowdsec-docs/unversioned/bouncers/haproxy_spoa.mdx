...
:::tip Prerequisites
Bot detection is an AppSec feature. First enable AppSec forwarding as described above, then follow the [bot detection guide](/docs/next/appsec/bot_detection/intro) to install `crowdsecurity/appsec-bot-challenge` and load the `crowdsecurity/appsec-bot-*` AppSec configs.
:::
...
Add the challenge ACL, response routing, and backend alongside your regular SPOE
configuration:
...
frontend http-in
    ...
    # Keep challenge follow-up requests out of the normal SPOE groups. They are
    # handled by the bouncer's challenge HTTP listener instead.
    acl body_within_limit req.body_size -m int le 51200
    ...
    # Required for /crowdsec-internal/challenge/*: this path bypasses SPOE, so
    # the bouncer needs HAProxy to pass the real client IP explicitly.
