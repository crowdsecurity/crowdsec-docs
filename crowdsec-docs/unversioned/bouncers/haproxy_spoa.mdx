...
The HAProxy SPOA bouncer supports the **CrowdSec Application Security Component (WAF)**, which inspects HTTP requests inline and blocks **SQL injection, XSS, path traversal**, and exploited CVEs.

**Enable in three steps:**

```bash
# Step 1 — Install AppSec rules on the Security Engine
...
# Step 2 — Enable AppSec forwarding in the SPOA bouncer config
# Add to /etc/crowdsec/bouncers/crowdsec-spoa-bouncer.yaml:
#   appsec_url: "http://127.0.0.1:7422"

# Step 3 — Restart the bouncer
sudo systemctl restart crowdsec-spoa-bouncer
```

For a full walkthrough with testing and monitoring, see the [**HAProxy (SPOA) WAF Quickstart**](/docs/next/appsec/quickstart/haproxy_spoa).
:::
