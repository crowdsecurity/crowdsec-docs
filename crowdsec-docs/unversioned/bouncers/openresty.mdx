...
description: "Run CrowdSec inside OpenResty's Lua runtime: IP blocking, CAPTCHA rendering and AppSec (WAF) inspection, with no separate proxy layer to deploy."
sidebar_position: 1
---
...
<BouncerHeader
  ...
  featuresSupport="waf, livemode, streammode, metrics, mtls, captcha, botdetection"
  githubUrl="https://github.com/crowdsecurity/cs-openresty-bouncer"
  hubUrl="https://app.crowdsec.net/hub/author/crowdsecurity/remediation-components/cs-openresty-bouncer"
/>
...
- Ban remediation (can ban an IP address by redirecting it or returning a custom HTML page)
- Captcha remediation (can return a captcha)
- Works with IPv4/IPv6
- Support IP ranges (can apply a remediation on an IP range)
- Application Security Component (forward the request to the CrowdSec Application Security Component and block if necessary)
...
[Set up the CrowdSec repositories](/u/getting_started/installation/linux#repository-installation).
...
[The openresty linux packages](https://openresty.org/en/linux-packages.html) don't include any `conf.d/` directory for custom configurations.
:::
...
The AppSec-related knobs in `/etc/crowdsec/bouncers/crowdsec-openresty-bouncer.conf` are documented in the [Configuration Reference](#configuration-reference) below (all `APPSEC_*` entries).
...
Any `/etc/crowdsec/bouncers/crowdsec-openresty-bouncer.conf.local` content will take
precedence over `/etc/crowdsec/bouncers/crowdsec-openresty-bouncer.conf`. Not all fields
have to be present in this `.local` file.
...
The AppSec-related options in `/etc/crowdsec/bouncers/crowdsec-openresty-bouncer.conf`:
...
You can add a decision with type `captcha` to check that it works correctly:
...
The best way to migrate from the crowdsec-openresty-bouncer v0.\* to v1 is to reinstall the bouncer. Many new configuration options are now available, and some have been removed.
...
Generated with the [`sudo cscli bouncers add`](/u/bouncers/intro#generate-an-api-key-for-your-bouncer) command.
...
- live: The component will query the local API for each request (if the IP is not in cache) and will store the IP in cache for X seconds (`CACHE_EXPIRATION` parameter).

:::note
The timer that pulls the local API will be triggered after the first request.
:::
...
The locations to exclude while bouncing. It is a list of locations, separated by commas.
...
The cache expiration, in seconds, for IPs that the component stores in cache in `live` mode.
...
The update frequency, in seconds, to pull new/old IPs from the CrowdSec local API.
...
If nothing is specified, it will return a 403.
...
The time for which the captcha will be validated. After this duration, if the decision is still present in CrowdSec local API, the IP address will get a captcha again.
...
Behavior when the AppSec Component returns a 500: the request can either pass through or be denied.
