...
<BouncerHeader
  ...
  githubUrl="https://github.com/crowdsecurity/cs-custom-bouncer"
  hubUrl="https://app.crowdsec.net/hub/author/crowdsecurity/remediation-components/cs-custom-bouncer"
/>
...
bin_args: []
feed_via_stdin: false # Invokes the binary once and feeds incoming decisions to its stdin.
total_retries: 0 
...
This is only applicable if you have configured your [profiles](/docs/next/local_api/profiles/format#decisions) to make use of scopes but it is general best practice to set it to `["Ip"]` by default if that is the script's aim.
...
If the binary exits for any reason, it will be reinvoked up to `total_retries` times. If the maximum number of retries is exhausted, the bouncer will quit.
...
```
 ...
 - `origin`: origin of the decision. Can be `crowdsec`, `cscli`, `cscli-import`, `CAPI`, `lists`.
...
Array of arguments to pass to the script that will be invoked.
...
Indicates whether the script will be fed via STDIN or via its arguments.
...
The keys for hashing a decision are its `Type` (eg `ban`, `captcha` etc) and `Value` (eg `192.168.1.1`,  `CH` etc).

### `piddir`
> string

:::warning
This field has been deprecated and is ignored by the component.
:::
