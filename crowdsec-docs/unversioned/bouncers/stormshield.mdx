...
<BouncerHeader
  ...
  githubUrl="https://github.com/crowdsecurity/cs-stormshield-bouncer"
/>

# Sync CrowdSec IP Bans to Stormshield Appliances
## Overview

The Stormshield Remediation Component syncs IP bans from CrowdSec with Stormshield appliances.
...
See how to enable ssh access on the firewall [here](https://documentation.stormshield.eu/SNS/v4/en/Content/Basic_Command_Line_Interface_configurations/Enable_ssh_access_with_password.htm). Usually the username and password are the same as for the web interface.
...
crowdsec:
  ...
  key_path: ""  # Used for TLS authentication with CrowdSec LAPI
...
Ignore IPs banned for triggering scenarios that do not contain any of the provided words. Example value ["ssh", "http"]
...
Only include IPs banned due to decisions originating from the provided sources. eg value ["cscli", "crowdsec"]
