...
description: "Filter traffic at Envoy Gateway through CrowdSec's external authorization service, which resolves IP decisions and calls the AppSec (WAF) engine per request."
---
...
<BouncerHeader
  ...
  typeLabel="Third Party Bouncer"
  imgSrc="/img/crowdsec_http.svg"
  description="External authorization bouncer for Envoy Gateway with WAF support."
  featuresSupport="streammode, metrics, prometheus, waf, captcha, mtls"
  githubUrl="https://github.com/kdwils/envoy-proxy-crowdsec-bouncer"
/>
...
If you enable [bot detection](/docs/next/appsec/bot_detection/intro), Envoy
Gateway must forward challenge submission request bodies to the external auth
service. The WAF quickstart shows the required
`SecurityPolicy.spec.extAuth.bodyToExtAuth.maxRequestBytes` setting.
:::
...
1. It is assumed that you already have:
   - A working CrowdSec
     [Security Engine](/docs/next/intro) installation. For a Kubernetes install
     quickstart, refer to
