...
description: "Enforce CrowdSec decisions in AWS WAF: the bouncer keeps IP sets and ACL rules in sync so malicious IPs are blocked at the CloudFront or ALB edge."
sidebar_position: 1
---
...
<BouncerHeader
  ...
  githubUrl="https://github.com/crowdsecurity/cs-aws-waf-bouncer"
  hubUrl="https://app.crowdsec.net/hub/author/crowdsecurity/remediation-components/cs-aws-waf-bouncer"
/>
# Block Malicious IPs on AWS WAF with CrowdSec
## Overview

The `crowdsec-aws-waf-bouncer` automatically adds rules to an AWS WAF ACL and manages IPSet contents to apply decisions taken by CrowdSec.
...
The default Docker timeout of 10s before sending a `SIGKILL` to the process might not always be enough.
...
Not required when scope is `CLOUDFRONT`.
...
Different IPSets are used for ban and captcha.
...
For reference, a simple match on an IP with no custom header will cost 1 WCU per IPSet created by the bouncer, or 5 WCU if you are getting the source IP from a header.
...
### `remove_sets_on_start`

> boolean

> Environment variable: `BOUNCER_WAF_CONFIG_X_CLEAN_ON_START`

Whether or not to remove any sets matching the current prefix in the configuration on startup.

Be careful if you are running multiple instances of the bouncer and using common prefixes, as this could lead to a bouncer deleting the other's sets.

## IAM Permissions

Because the component needs to interact with AWS resources, it needs the proper permissions.
