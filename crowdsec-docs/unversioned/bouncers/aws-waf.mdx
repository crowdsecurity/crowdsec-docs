...
As the remediation does not manage your WAF ACLs, you will need to have existing ACLs already associated with your AWS resources for the remediation to work properly.
The remediation supports the `ban` and `captcha` decisions type and can be configured to fall back to one of those for decisions of unknown type.
...
The remediation will create all required resources and associate them with your existing ACLs based on your provided configuration.

As the resources will incur an AWS cost, the remediation will remove everything it created when stopping.
...
You will need to edit `/etc/crowdsec/bouncers/crowdsec-aws-waf-bouncer.yaml` to configure the ACLs you want the remediation to use.
...
Optionally, the remediation can also be configured using only environment variables.
...
How often the remediation will contact LAPI to update its content.
...
Prefix for all the ipsets the remediation will create.
...
Capacity of the rule group created by the remediation.
...
Whether or not to sample requests from the rule groups created by the remedation.

## IAM Permissions

Because the remediation component needs to interact with AWS resources, it need the proper permissions.
...
For the other permissions, we recommend to restrict the resources to only the WebACL the remediation component is configured to interact with and the rule groups/ipsets the remediation component will create. 