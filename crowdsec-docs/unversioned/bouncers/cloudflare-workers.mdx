...
<img src={useBaseUrl('/img/bouncer/cloudflare-worker/cf-autonomous-init.png')} width="50%" alt="cloudflare-worker remediation component autonomous init view"/>
...
<img src={useBaseUrl('/img/bouncer/cloudflare-worker/cf-autonomous-sync.png')} width="50%" alt="cloudflare-worker remediation component autonomous sync view"/>
...
<img src={useBaseUrl('/img/bouncer/cloudflare-worker/cf-autonomous-init.png')} width="50%" alt="INIT Architecture"/>
...
<img src={useBaseUrl('/img/bouncer/cloudflare-worker/cf-autonomous-sync.png')} width="50%" alt="SYNC Architecture"/>

4. This mode reduces infrastructure requirements as no persistent daemon is needed

<img src={useBaseUrl('/img/bouncer/cloudflare-worker/cf-autonomous-architecture.png')} width="100%" alt="Autonomous Architecture"/>
...
If your Cloudflare token has permission on D1, the remediation component will automatically create a database to track:
 - Number of requests processed
 - Number of requests blocked

Metrics are stored in a D1 database. If your token does not have access to D1, a warning will be logged and metrics will be disabled.
...
- Metrics are collected and stored in the D1 database
...
cloudflare_config:
    ...
    ignore_binding_errors_on_deploy: false # If true, worker route binding errors during deployment will only log warnings instead of failing
...
#### `ignore_binding_errors_on_deploy`
> boolean

If true, worker route binding errors during deployment will only log warnings instead of failing.
Default to `false`.
