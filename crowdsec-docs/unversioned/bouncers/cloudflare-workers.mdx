...
1. Two Cloudflare Workers are deployed:
   - `crowdsec-cloudflare-worker-bouncer` - applies cached decisions to incoming requests (same as daemon mode)
   - `decisions-sync-worker` - periodically fetches and caches security decisions using Cloudflare scheduled tasks
2. The Go process only needs to run once for initial setup and configuration deployment
3. All decision synchronization is handled automatically by the `decisions-sync-worker` using Cloudflare's scheduled tasks
