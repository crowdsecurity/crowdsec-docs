...
description: "A WordPress plugin that turns away malicious visitors, shows CAPTCHA pages and forwards requests to the AppSec (WAF) engine, all managed from wp-admin."
---
...
<BouncerHeader
  ...
  featuresSupport="livemode, streammode, mtls, waf, metrics, captcha"
  githubUrl="https://github.com/crowdsecurity/cs-wordpress-bouncer"
  hubUrl="https://app.crowdsec.net/hub/author/crowdsecurity/remediation-components/cs-wordpress-bouncer"
/>
...
As an existing CrowdSec Console user, you can send the blocklists you already have access to directly to your WordPress site without using a Security Engine:
...
<img src={useBaseUrl('/img/bouncer/wordpress/screenshots/config-bouncing.png')} alt="Bouncing" title="Bouncing" 
...
<img src={useBaseUrl('/img/bouncer/wordpress/screenshots/config-usage-metrics.png')} alt="Remediation metrics" title="Remediation metrics" />
...
Minimum 1 second.  Note that this setting cannot be applied in stream mode.
...
If enabled, bounced IPs will be geolocated and the final remediation will take into account any country related decision.
...
Set 0 to disable caching. Enabling this will avoid multiple calls to the geolocation system (e.g. MaxMind database).
...
When this mode is enabled, you will see every unexpected bouncing error in the browser.
...
If you are using Apache, the plugin root folder already contains the required `.htaccess` file to protect log,  
...
By default, this extension bounces every web request that goes through the standard WordPress core loading process.
This implies that if another php public script is called (any of your custom public php script for example) or if you are using a plugin that bypasses the WordPress core loading process (as the [WP Super Cache plugin](https://wordpress.org/plugins/wp-super-cache/) in Simple mode for example), bouncing will not be effective.
