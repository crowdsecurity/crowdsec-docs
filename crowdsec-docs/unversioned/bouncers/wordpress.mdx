...
## WordPress Plugin
The CrowdSec plugin can both be used to directly use CrowdSec Blocklists within WordPress as well as coupled with the CrowdSec Security Engine detecting malicious bots and attacks.
In this Documentation we will explain 3 use cases:
- **Instant WordPress Blocklist**: Block known WordPress attackers in just a few clicks!
- **Plug the CrowdSec Blocklist as a Service**: For existing CrowdSec console users, you can plug your custom [Blocklist Integration](#plug-crowdsec-blocklist-as-a-service) into the plugin too.
- **CrowdSec Bouncer**: Connect your Security Engine to WordPress and block malicious bots and attacks detected on your server in real time.
    Follow the [full user guide](#crowdsec-wordpress-bouncer-plugin---user-guide) to understand how the plugin interracts with the CrowdSec Security Engine and how to configure it.

## Instant WordPress Blocklist
This feature, exclusive to the WP plugin gives you access to CrowdSec's WordPress Blocklist in just a few clicks.
This blocklist contains the 10k most aggressive IPs currently attacking WordPress sites.
Using it will help you block those attackers before they even try funny business on your site. (Doesn't dispense of the need to keep your WordPress and plugins up to date!)

Simply subscribe to this blocklist <a class="button tw-bg-primary tw-text-white hover:tw-text-white/80 hover:tw-bg-primary/80 active:tw-bg-primary/90" href="https://buy.stripe.com/00g3cIcu59JVfewaES">via this link</a>, or the one found in the plugin, and you'll receive access to it within 24hours.
The subscription form only asks you for your email (to receive the access link) and your WordPress site URL (to ensure you are a real WordPress site).
Optionnaly, If you are already a CrowdSec user, indicate your **organisation id** so we can give you access to the blocklist in your CrowdSec console.

Follow the instructions bellow based on your use case:
### 1. Instant WordPress Blocklist directly connected in the plugin
- **Step 1**: Install the CrowdSec WordPress plugin
- **Step 2**: Go to the plugin settings and click on the "Instant WordPress Blocklist" <a class="button tw-bg-primary tw-text-white hover:tw-text-white/80 hover:tw-bg-primary/80 active:tw-bg-primary/90" href="https://buy.stripe.com/00g3cIcu59JVfewaES">via this link</a>
- **Step 3**: Fill in the form with your email and WordPress URL
- **Step 4**: You'll receive the access link to the blocklist in your email within 24 hours
- **Step 5**: Go back to the plugin settings and paste the access link in the fields like so:
[Fill the fields like so](!/img/bouncer/wordpress/screenshots/plugin-blaas-fields.png) 

### 2. Instant WordPress Blocklist connected to your CrowdSec console
- **Step 1**: We assume you already have the plugin installed and configured to connect to your CrowdSec Security Engine
- **Step 2**: Go to the plugin settings and click on the "Instant WordPress Blocklist" or <a class="button tw-bg-primary tw-text-white hover:tw-text-white/80 hover:tw-bg-primary/80 active:tw-bg-primary/90" href="https://buy.stripe.com/00g3cIcu59JVfewaES">via this link</a>
- **Step 3**: Fill in the form with your email and WordPress URL and mention the **organisation id** of your CrowdSec console (you can find it in the settings section or the console
- **Step 4**: You'll receive the access confirmation in your email within 24 hours
- **Step 5**: Your organisation will now benefit from the wordpress blocklist access and your security engines will already be subscribed to it.
              You can check the status of your subscription in the [console's blocklist/subscriptions section](https://app.crowdsec.net/blocklists/subscriptions).

## Plug CrowdSec Blocklist as a Service
If you want to send blocklists directly from the CrowdSec console to your WordPress site, without using a security engine you can do so in two easy steps:

### Step 1: Create a Blocklist as a Service endpoint of type Remediation Component Integration
- Follow [this guide](/u/integrations/remediationcomponent) to create
- Subscribe to the blocklsits of your choice as indicated in [these instructions](/u/console/blocklists/subscription)

### Step 2: Configure the plugin
- Use the URL and API-Key/Token provided on creation of your integration endpoint
[Fill the fields like so](!/img/bouncer/wordpress/screenshots/plugin-blaas-fields.png) 

### Specific WP plugin configurations when using a direct Blocklist as a Service integration
As the plugin won't be interacting with a security engine some features of the plugin become useless and some need to be set right:
1. Use Stream mode !
  - Go to Crowdsec Advanced settings and enable Stream mode
  - This is the only mode that will work with a direct Blocklist as a Service integration.
[Plugin Stream mode](!/img/bouncer/wordpress/screenshots/plugin-stream-mode.png)  
2. Usage Metrics will appear in the plugin, but won't be sent to the console
3. AppSec won't be available

## CrowdSec WordPress Bouncer Plugin - User Guide

### Description

The `CrowdSec Bouncer` plugin for WordPress has been designed to protect WordPress websites from various malicious 
activities by using [CrowdSec](https://www.crowdsec.net/) technology.
...
CrowdSec is only in charge of the "detection", and won't block anything on its own. You need to deploy a remediation to 
"apply" decisions.
...
When a user is suspected by CrowdSec to be malevolent, this component will either send him/her a captcha to resolve or
