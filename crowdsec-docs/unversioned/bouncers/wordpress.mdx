...
- [**Blocklist as a Service Integration**](#plug-crowdsec-blocklist-as-a-service): Existing CrowdSec users, use your selected blocklists directly in WordPress.
...
- **1**. Install the CrowdSec WordPress plugin. 
  - From the WordPress admin panel, navigate to `Plugins` > `Add New`, search for "CrowdSec", and click `Install Now`.
  - Alternatively it's avalable [here](https://wordpress.org/plugins/crowdsec/)
...
- Usage Metrics appear locally but are not sent to the CrowdSec console.
