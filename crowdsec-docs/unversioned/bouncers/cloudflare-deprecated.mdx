...
<p align="center">
  <img src={useBaseUrl('/img/crowdsec_cloudflare.svg')} width="400" height="300" />
</p>
...
<BouncerHeader
  title="Cloudflare Bouncer (Deprecated)"
...
Please configure your server to log real visitor IPs rather than Cloudflare IPs, so CrowdSec can function properly. See how [here](https://support.cloudflare.com/hc/en-us/articles/200170786-Restoring-original-visitor-IPs)

:::

:::info

If your component is not installed on the same machine as LAPI, don't forget to set the `crowdsec_lapi_url` and `crowdsec_lapi_key` in the configuration file `/etc/crowdsec/bouncers/crowdsec-cloudflare-bouncer.yaml`

:::

:::note

You need to run `sudo crowdsec-cloudflare-bouncer -d` to clean up existing Cloudflare components created by the component before editing the config files.
...
sudo ./install.sh
```
The rest of the steps are the same as in the method above.
...
```bash
sudo cscli -oraw bouncers add cloudflarebouncer # the -oraw flag can be omitted for human-friendly output.
```
...
Ignore IPs banned for triggering scenarios that do not contain any of the provided words.
...
Only include IPs banned due to decisions originating from the provided sources.
...
The following table maps each decision type to its destination IP list.
...
The following table maps each decision type to its destination IP list. Lists are created according to zone actions.
