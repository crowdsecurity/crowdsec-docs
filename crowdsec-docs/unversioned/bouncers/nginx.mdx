...
:::tip Protect against web attacks — Enable the WAF (Recommended)
The Nginx Remediation Component can forward every HTTP request to the **CrowdSec Application Security Component (WAF)** for inline inspection. This adds protection against **SQL injection, XSS, path traversal**, and actively exploited CVEs — with zero false positives by default.

**Enable in two steps:**

```bash
# Step 1 — Activate AppSec on the Security Engine
sudo cscli collections install crowdsecurity/appsec-virtual-patching crowdsecurity/appsec-generic-rules
sudo sh -c 'cat > /etc/crowdsec/acquis.d/appsec.yaml << EOF
appsec_configs:
...
# Step 2 — Point the Nginx bouncer to the AppSec Component
echo "APPSEC_URL=http://127.0.0.1:7422" | sudo tee -a /etc/crowdsec/bouncers/crowdsec-nginx-bouncer.conf
sudo systemctl restart nginx
```

For the full walkthrough with testing and monitoring, see the [**Nginx/OpenResty WAF Quickstart**](/docs/next/appsec/quickstart/nginxopenresty).
:::
...
To make HTTP request in the component, you need to configure a resolver and ssl certifcates. Here is a [our example](#resolver-and-certificates).
...
The AppSec Component (WAF) inspects every HTTP request inline and blocks attacks before they reach your application.

**Step 1 — Set up the AppSec Component on the Security Engine** (if not already done):
...
**Step 2 — Enable forwarding in the Nginx bouncer** by adding these lines to `/etc/crowdsec/bouncers/crowdsec-nginx-bouncer.conf`:
...
**Test the WAF is working:**

```bash
# This request should be blocked (HTTP 403) — .env access is a known attack vector
curl -i http://localhost/.env
```

For a full walkthrough with monitoring and advanced profiles, see the [Nginx/OpenResty WAF Quickstart](/docs/next/appsec/quickstart/nginxopenresty).
