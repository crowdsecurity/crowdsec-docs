...
description: "Add IP blocking, CAPTCHA pages and AppSec (WAF) inspection to NGINX with CrowdSec's Lua module, in live mode or from a locally cached decision stream."
sidebar_position: 1
---
...
<BouncerHeader
  ...
  githubUrl="https://github.com/crowdsecurity/cs-nginx-bouncer"
  hubUrl="https://app.crowdsec.net/hub/author/crowdsecurity/remediation-components/cs-nginx-bouncer"
/>
...
::::warning
If you have Ubuntu 22.xx, you may have an issue with the `lua` module in nginx.  
Look at [FAQ for more information](#ubuntu-22xx-getting-lua-error).
::::

### Using packages

First, [set up the CrowdSec repositories](/u/getting_started/installation/linux#repository-installation).
...
:::warning
Upgrading from v0 to v1 introduces many changes. When prompted, install the package maintainer's version of the configuration to avoid breaking anything. Configuration migration might not be trivial.
:::
...
Not all fields have to be present in this `.local` file.
...
To make HTTP requests in the component, you need to configure a resolver and SSL certificates. Here is [an example](#resolver-and-certificates).
...
- `ngx.var.crowdsec_always_send_to_appsec`: set to 1, it will always send the request to AppSec, even if a decision already exists for the requesting IP
