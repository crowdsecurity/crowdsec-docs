...
:::tip AppSec Support
This bouncer supports the [AppSec Component](/docs/next/appsec/intro) for real-time WAF protection. After installing the bouncer, [enable AppSec](#enable-appsec-waf-protection) to get virtual patching and defense against known CVEs, SQL injection, XSS, and other application-layer attacks.

For a full walkthrough, see the [AppSec Quickstart for Nginx/OpenResty](/docs/next/appsec/quickstart/nginxopenresty).
:::
...
## Enable AppSec (WAF Protection)

The Nginx bouncer can forward HTTP requests to the CrowdSec AppSec Component for real-time inspection, giving you virtual patching and protection against known CVEs and generic attack patterns.

### 1. Install AppSec collections on the Security Engine

```bash
sudo cscli collections install crowdsecurity/appsec-virtual-patching crowdsecurity/appsec-generic-rules
```

### 2. Configure the AppSec acquisition
...
### 3. Enable AppSec in the bouncer

Add the following to `/etc/crowdsec/bouncers/crowdsec-nginx-bouncer.conf`:
...
:::info
For a more detailed guide including console integration and advanced configuration, see the [AppSec Quickstart for Nginx/OpenResty](/docs/next/appsec/quickstart/nginxopenresty).
:::
...
For a step-by-step guide to enable AppSec, see [Enable AppSec (WAF Protection)](#enable-appsec-waf-protection) above.
