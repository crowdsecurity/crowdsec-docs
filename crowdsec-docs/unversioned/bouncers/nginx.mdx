...
A lua remediation for nginx.

## How does it work ?

This remediation leverages nginx lua's API, namely `access_by_lua_block` to check the IP address against the local API.
...
:::warning
CrowdSec NGINX Remediation depends on `nginx lua5.1 libnginx-mod-http-lua luarocks gettext-base`.
...
The remediation NGINX configuration is located in `/etc/nginx/conf.d/crowdsec_nginx.conf` :
...
To make HTTP request in the remediation, you need to configure a resolver and ssl certifcates. Here is a [our example](#resolver-and-certificates).
...
If you want to use captcha with your Nginx Remediation, you must provide a Site key and Secret key in your remediation configuration. If you wish to use any other provider than recaptcha you must also provide a Captcha provider.
