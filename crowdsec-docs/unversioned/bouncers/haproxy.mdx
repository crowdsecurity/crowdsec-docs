...
<BouncerHeader
  ...
  githubUrl="https://github.com/crowdsecurity/cs-haproxy-bouncer"
/>
...
We strongly suggest that you use the [HAProxy SPOA Bouncer](/u/bouncers/haproxy_spoa).
...
Supported features:
 ...
 - Ban remediation (can ban an IP address by redirecting the client or returning a custom HTML page)
...
You must declare a backend for CrowdSec so its address can be resolved during the refresh task. The backend and the server must both be named `crowdsec`.
...
When using captcha, you must declare a backend for the captcha provider so its address can be resolved during captcha verification.
...
Generated with the [`sudo cscli bouncers add`](/docs/next/cscli/cscli_bouncers_add) command.
...
By default, the HTML template is located in `/var/lib/crowdsec/lua/haproxy/templates/ban.html`.
...
By default, the HTML template is located in `/var/lib/crowdsec/lua/haproxy/templates/captcha.html`.
