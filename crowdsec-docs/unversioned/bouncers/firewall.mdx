...
<BouncerHeader
  ...
  githubUrl="https://github.com/crowdsecurity/cs-firewall-bouncer"
  hubUrl="https://app.crowdsec.net/hub/author/crowdsecurity/remediation-components/cs-firewall-bouncer"
/>
...
Continue with the [pf setup section](#pf-setup-freebsd).


See as well [Manual Installation documentation below](#manual-installation)

## pf setup (FreeBSD) {#pf-setup-freebsd}

The `pf` mode relies on the `pfctl` command to alter `pf` tables. You must create the CrowdSec tables and reference them in your `pf.conf`.

### 1) Enable `pf`

`pf` is not always loaded/enabled by default.
...
### 2) Configure `/etc/pf.conf`

Add the CrowdSec tables and blocking rules (adjust to your existing ruleset):
...
```bash
sudo cscli bouncers add freebsd-pf-bouncer
```

Edit the bouncer configuration file on FreeBSD (`/usr/local/etc/crowdsec/bouncers/crowdsec-firewall-bouncer.yaml`) and set at least:
...
In "managed" mode (mode `nftables` or `iptables`), the component creates all the needed elements (rules, sets) and inserts the appropriate rules in nftables or iptables.
...
If you are running ipset only mode, crowdsec-firewall-bouncer tries parsing the output to produce metrics, but:
 - "managed" firewalls such as ufw might confuse the parser and lead to inconsistent metrics.
...
### `update_frequency`
> string (That is parseable by [time.ParseDuration](https://pkg.go.dev/time#ParseDuration))
...
Only get IPs banned for triggering scenarios containing any of the provided words.
...
### modes
 ...
 - mode `ipset` relies on `ipset` and only manages the contents of the sets (they need to exist at startup and will be flushed rather than created)
...
```
 ...
 You can refer to the step-by-step instructions for FreeBSD in the [pf setup section](#pf-setup-freebsd).
