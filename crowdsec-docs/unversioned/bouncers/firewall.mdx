...
CrowdSec remediation written in golang for firewalls.
...
In "managed" mode (mode `nftables` or `iptables`), remediation creates all the needed elements (rules, sets) and insert the appropriate rules in nftables or iptables.
...
In iptables set-only mode (mode `ipset`), the remediation only handles the **contents** of sets that are specified by `blacklists_ipv4` and `blacklists_ipv6`.
These sets must be created before starting the remediation, and it is the user's responsibility to create the associated iptables rules.
...
In nftables set only mode (mode `nftables` with `nftables.{ipv4,ipv6}.set-only` set to `true`), the remediation only manages the **contents** of the sets.
...
If `set-only` is set to true, the remediation will only manage the set contents.
