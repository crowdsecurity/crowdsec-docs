...
<BouncerHeader
  ...
  imgSrc="/img/crowdsec_cloudflare.svg"
  description="Syncs CrowdSec decisions with Cloudflare Workers to block or challenge traffic on your zones."
  featuresSupport="mtls, streammode, captcha"
  githubUrl="https://github.com/crowdsecurity/cs-cloudflare-worker-bouncer-install"
  hubUrl="https://app.crowdsec.net/hub/author/crowdsecurity/remediation-components/cs-cloudflare-worker-bouncer"
/>
...
- A **[Security Engine](/docs/next/intro)** you run in your own infrastructure: the Sync Worker connects to your LAPI to pull decisions directly.
...
:::info Decision source and setup method
**Self-Hosted Installer** works best with a **Blocklist Integration Endpoint** — no infrastructure to expose, the Sync Worker pulls decisions directly from CrowdSec.

Using a **self-hosted Security Engine (LAPI)** as the decision source requires the LAPI to be reachable from Cloudflare's network, which means exposing it publicly. If that's a concern:
- **Premium users** can enable [Remediation Sync](/u/console/remediation_sync) on their Console integration — decisions are forwarded from the LAPI without exposing it to the internet.
- Alternatively, use the **CLI Bouncer Daemon** running alongside your LAPI, so the bouncer pulls decisions locally and pushes them to Cloudflare without any inbound exposure.
:::
...
- [Installing the bouncer and binding your zones via the Installer GUI](#configure-via-the-installer-ui)
  - Easy one-click or batch actions on your selected zones
...
Toggle **Supports CAPTCHA** on a zone to enable Turnstile for it and pick a widget mode (managed, non-interactive, invisible). Protected zones with Turnstile show a **SUPPORTS CAPTCHA** badge. You can toggle it on or off after installation as well.
...
## 🔧 CLI Bouncer Setup {#cli-bouncer-setup}
...
## ⚙️ Setting Up the Worker Route Fail Mode {#setting-up-the-worker-route-fail-mode}
