...
A remediation that syncs the decisions made by CrowdSec with CloudFlare's firewall. Manages multi user, multi account, multi zone setup. Supports IP, Country and AS scoped decisions.

:::warning
Currently use of this remediation is discouraged if you are on free plan with Cloudflare due to rate limitations. We are working to resolve this with Cloudflare support.
...
If your remediation is not installed on the same machine than LAPI, don't forget to set the `crowdsec_lapi_url` and `crowdsec_lapi_key` in the configuration file `/etc/crowdsec/bouncers/crowdsec-cloudflare-bouncer.yaml`

:::

:::note

You need to run `sudo crowdsec-cloudflare-bouncer -d` to cleanup exisiting cloudflare components created by remediation before editing the config files.
...
The remediation creates Cloudflare infra (IP lists, rules etc) according to your config file. 
...
After upgrading the remediation from v0.0.X to v0.1.Y , run the following commands to migrate to `managed_challenge`.
...
2. Go to [Tokens](https://dash.cloudflare.com/profile/api-tokens) and create the token. The remediation requires the follwing permissions to function.
...
The remediation binary has built in helper scripts to do various operations.
...
The URL of CrowdSec LAPI. It should be accessible from the remediation.
...
The remediation will poll the CrowdSec every `update_frequency` interval.
...
This block contains config for each zone to be managed by the remediation. The zone must be accessible from the parent account.
