...
<BouncerHeader
  ...
  githubUrl="https://github.com/crowdsecurity/cs-blocklist-mirror"
  hubUrl="https://app.crowdsec.net/hub/author/crowdsecurity/remediation-components/cs-blocklist-mirror"
/>
...
The URL of CrowdSec LAPI. It should be accessible from whichever network the component has access to.
...
```bash
sudo cscli -oraw bouncers add blocklistMirror # the -oraw flag can be omitted for human-friendly output.
```
...
The component can expose the blocklist in the following formats. You can configure the format of the blocklist by setting its `format` parameter to any of the supported formats described below.
