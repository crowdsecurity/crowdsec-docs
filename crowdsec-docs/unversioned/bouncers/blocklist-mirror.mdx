...
This remediation exposes CrowdSec's active decisions via provided HTTP(S) endpoints in pre-defined formats. It can be used by network appliances which support consumption of blocklists via HTTP.
...
The remediation will poll the CrowdSec LAPI every `update_frequency` interval.
...
The bindable address and port for the remedation to listen on EG: `127.0.0.1:41412`
