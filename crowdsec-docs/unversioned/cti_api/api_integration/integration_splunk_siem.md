...
If an IP address is not found in the local CrowdSec CTI database, the app automatically falls back to the bundled **CIRCL** ([data.public.lu](https://data.public.lu/en/datasets/)) MMDB dataset to enrich the event with at least country and AS/ASN information. This ensures that Offline replication always returns basic geolocation and network owner context, even when CrowdSec CTI has no match for a given IP.
