...
<Tabs groupId="deployment">
  ...
  **To check your acquisition config:**
  ...
  In Kubernetes, CrowdSec reads logs directly from pods using the Kubernetes API.
  ...
  agent:
    acquisition:
      - namespace: your-namespace
        podName: your-pod-*
        program: nginx  # Must match parser name
  ```

  **Common issues:**
  - **Wrong namespace or pod names**: Verify pods exist with `kubectl get pods -n <namespace>`
  - **Incorrect program name**: The `program` field must match an available parser (nginx, traefik, apache, etc.)
...
<Tabs groupId="deployment">
  ...
  **Common issues:**
  - **Missing credentials**: If `online_api_credentials.yaml` is missing:
    ...
    sudo systemctl reload crowdsec
    ```
  - **Firewall blocking**: Ensure outbound access to `api.crowdsec.net` (HTTPS/443)
