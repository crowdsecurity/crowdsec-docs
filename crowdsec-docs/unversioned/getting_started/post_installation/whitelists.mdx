...
On Premium plans, [Decision Management](console/decisions/decisions_management.mdx) lets you manage decisions via the [Console](https://app.crowdsec.net/).
...
You can see the full list of `cscli allowlists` commands [here](/docs/next/cscli/cscli_allowlists). 
...
If you want to whitelist an IP address that is not static, you need to use an external DDNS service to resolve the IP address and then use the Postoverflow whitelist to whitelist the resolved IP address.
...
:::info
This example uses Nginx logs; adjust the commands to point to your own log file.
:::
...
## Whitelisted but there is still a decision?
...
If you have created and tested your first whitelist, [return to the post-installation steps](/getting_started/next_steps.mdx#2-whitelists-).
