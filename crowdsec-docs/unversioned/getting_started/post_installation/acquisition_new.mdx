...
<FormattedTabs
  bash="cscli collections install <collection_name>"
  powershell="cscli.exe collections install <collection_name>"
...
:::info
You can add acquisitions via the `acquis.d` directory, which avoids editing the main configuration file.
:::
...
:::info
On Windows, the `acquis.d` directory is not loaded by default: add `acquisition_dir: C:\ProgramData\CrowdSec\config\acquis.d` under `crowdsec_service` in `C:\ProgramData\CrowdSec\config\config.yaml`.
:::
...
<FormattedTabs
  ...
  k8s="helm upgrade --reset-then-reuse-values crowdsec crowdsec/crowdsec -n crowdsec -f values.yaml"
/>
