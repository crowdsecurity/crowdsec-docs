...
If you add a health check, use `cscli lapi status` — not `cscli capi status`. The latter calls the Central API on every check and can trigger [CAPI rate limiting](/u/troubleshooting/capi_403) if the interval is short.
...
## Choose a Remediation Component

:::warning
The Security Engine by itself is a detection engine -- it will not block anything. You need to add a [Remediation Component](/bouncers/intro.md) to enforce decisions.
:::

:::tip Protecting a web application?
Use a **WAF-capable** bouncer to get real-time WAF protection and virtual patching. For Docker and Kubernetes environments, the [Traefik bouncer plugin](/bouncers/traefik.mdx) is a natural fit. If you use Nginx as a reverse proxy, see the [Nginx bouncer](/bouncers/nginx.mdx).

See the [full bouncer selection guide](/bouncers/intro.md) for all available options.
:::

For non-web services (SSH, databases, SMTP), use the [firewall bouncer](/bouncers/firewall.mdx) for IP-level protection.
