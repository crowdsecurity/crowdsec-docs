...
The available `values.yaml` options are described [here](/docs/next/configuration/values_parameters). You can start with this
example:
...
config:
  config.yaml.local: |
    api:
      client:
        # the log processor will unregister itself from LAPI when stopping.    
...
named `type`, with the type being the parsed log program (e.g. nginx, traefik). A
...
To avoid storing secrets in your `values.yaml`, use Kubernetes secrets:

```bash
kubectl create secret generic crowdsec-keys \
  --from-literal=ENROLL_KEY=<enroll_key> \
  --from-literal=BOUNCER_KEY_traefik=<bouncer_key> \
  -n crowdsec
```
