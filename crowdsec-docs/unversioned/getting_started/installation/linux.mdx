...
>
  <TabItem value="debian">
    ...
    <p>In order to install a deb repo, first you need to install the GPG key that is used to sign repository metadata. This will change depending on whether or not your apt version is >= v.1.1. You can check this by running:</p>
    ...
    <p>Run this command to update your local APT cache:</p>
    <CodeBlock className="language-bash">sudo apt update</CodeBlock>
...
### Choose and Install a Remediation Component

:::warning
The Security Engine by itself is a detection engine -- it will not block anything. You need to install a [Remediation Component](/bouncers/intro.md) to enforce decisions.
:::

:::tip Protecting a web application?
If you are running a web server or reverse proxy (Nginx, OpenResty, Traefik, HAProxy), install the matching **WAF-capable** bouncer instead of the firewall bouncer. This gives you real-time WAF protection, virtual patching, and defense against application-layer attacks (SQLi, XSS, CVE exploits).

- **Nginx** -- [crowdsec-nginx-bouncer](/bouncers/nginx.mdx)
- **OpenResty** -- [crowdsec-openresty-bouncer](/bouncers/openresty.mdx)
- **Traefik** -- [traefik-bouncer-plugin](/bouncers/traefik.mdx)
- **HAProxy** -- [cs-haproxy-spoa-bouncer](/bouncers/haproxy_spoa.mdx)

See the [full bouncer selection guide](/bouncers/intro.md) for details.
:::

#### Firewall Bouncer (Infrastructure Protection)

For non-web services such as SSH, databases, or SMTP, the firewall bouncer provides broad IP-level protection:
