...
If you are running a web server or reverse proxy (Nginx, OpenResty, Traefik, HAProxy), install the matching **AppSec-capable** bouncer instead of the firewall bouncer. This gives you real-time WAF protection, virtual patching, and defense against application-layer attacks (SQLi, XSS, CVE exploits).
