...
```bash
cscli machines list  # ignore the warning, it is expected at this point
```
...
- Replace the content of the config/acquis.yaml file (with your editor of choice) with the following:
```yaml
filenames:
...
CrowdSec collections are frequently updated with new behavior detections.  
The CrowdSec team creates and curates community scenarios so that users benefit from the latest vulnerability detections.
...
```
### Make sure log rotation does not break acquisition
...
```
- Note that the `dsn` parameter takes the `file://` protocol and an **absolute path**
...
- Note that if you want to do it yourself the process is:
  ...
  - Move it to the systemd user folder (`~/.config/systemd/user/`)
...
But to check that it's working, visit your website
...
Now that CrowdSec is running and detecting bad behaviors, alerts are raised and decisions to block bad actors are stored in the local database.  
