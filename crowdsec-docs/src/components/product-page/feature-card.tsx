	...
	return (
		...
		>
			<div style={{ fontSize: 28, marginBottom: 12, lineHeight: 1 }}>
				{icon.startsWith("/") ? <img src={icon} alt={title} style={{ height: 32, width: 32, border: "none" }} /> : icon}
			</div>
			<div
				style={{
					fontSize: 14.5,
			...
			<p style={{ fontSize: 13, color: "var(--cs-ink-dim)", margin: 0, lineHeight: 1.55 }}>{description}</p>
		</Link>
	);
};
