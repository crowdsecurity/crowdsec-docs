...
export const firewallIntegrations = [
  { name: 'Checkpoint', slug: 'checkpoint', href: '/u/integrations/checkpoint', desc: 'Custom Intelligence (IoC) Feeds',     color: '#cc0000' },
...
export default function IntegrationTile({ name, slug, href, desc, color, children }: IntegrationTileProps) {
  ...
  return (
    <div
      ...
      onMouseEnter={e => {
        (e.currentTarget as HTMLDivElement).style.boxShadow = `0 4px 12px ${color}30`;
        (e.currentTarget as HTMLDivElement).style.borderColor = `${color}70`;
      }}
      onMouseLeave={e => {
        (e.currentTarget as HTMLDivElement).style.boxShadow = 'none';
        (e.currentTarget as HTMLDivElement).style.borderColor = 'var(--ifm-color-emphasis-200)';
      }}
    >
      <div
        style={{
