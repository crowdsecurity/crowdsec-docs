	...
	return (
		<section>
			<div className="tw-text-left">
				<h2 className="tw-mb-1 tw-text-white">Select your environment</h2>
			...
			<div className="tw-grid tw-grid-cols-1 md:tw-grid-cols-3 tw-gap-4 tw-mt-4">
				{staticData.map((props) => (
					<Link key={props.text} href={props.link} className="tw-border-0 hover:tw-no-underline">
						<div
							className={
						...
						>
							<span className="tw-border-2 tw-border-gray-700 tw-bg-alpa-primary tw-w-12 tw-h-12 tw-rounded-lg tw-inline-flex tw-items-center tw-justify-center">
								{(!Array.isArray(props.icon) && <props.icon className="icon icon-xl" />) || (
									// biome-ignore lint/suspicious/noExplicitAny: We need to cast to any for CIcon to work with icons
									<CIcon icon={props.icon as any} size="xl" />
								)}
							</span>

							<span className={"tw-text-md tw-font-semibold tw-flex-1 tw-text-left"}>{props.text}</span>
						</div>
					</Link>
				))}
			</div>
