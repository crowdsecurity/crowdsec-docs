  ...
  return (
    <div className={styles.wrap}>
      ...
      >
        ...
        <text x="26" y="35" textAnchor="middle" fontSize="14" fontWeight="700" fill="#0A1120">C</text>
        ...
        {[60, 90, 120, 150, 180, 210].map((y, i) => (
          <g key={y}>
            ...
            <rect x="18" y={y + 5} width="12" height="12" rx="3" fill={i === 1 ? 'var(--cs-orange)' : 'var(--cs-ink-mute)'} opacity={i === 1 ? 0.9 : 0.4} />
            <rect x="36" y={y + 7} width={[55, 70, 48, 62, 44, 58][i]} height="8" rx="3" fill={i === 1 ? 'var(--cs-orange)' : 'var(--cs-ink-mute)'} opacity={i === 1 ? 0.8 : 0.35} />
          </g>
        ))}
        ...
        {[
          ...
          <g key={card.x}>
            ...
            <text x={card.x + 14} y="77" fontSize="9" fill="var(--cs-ink-mute)" fontFamily="monospace">{card.label.toUpperCase()}</text>
            <text x={card.x + 14} y="104" fontSize="22" fontWeight="700" fill={card.color}>{card.value}</text>
            {/* Sparkline */}
            <polyline
        ...
        {/* Main chart area */}
        <rect x="172" y="140" width="390" height="220" rx="8" fill="var(--cs-surface-2)" stroke="var(--cs-border)" strokeWidth="1" />
        <text x="186" y="160" fontSize="9" fill="var(--cs-ink-mute)" fontFamily="monospace">ALERTS OVER TIME</text>
        {/* Chart bars */}
        ...
        {['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'].map((m, i) => (
          <text key={m} x={195 + i * 30} y="335" fontSize="7" fill="var(--cs-ink-mute)" textAnchor="middle">{m}</text>
        ))}

        {/* Right panel */}
        <rect x="574" y="140" width="214" height="220" rx="8" fill="var(--cs-surface-2)" stroke="var(--cs-border)" strokeWidth="1" />
        <text x="588" y="160" fontSize="9" fill="var(--cs-ink-mute)" fontFamily="monospace">TOP THREATS</text>
        {[
          ...
          <g key={row.label}>
            <text x="588" y={182 + i * 34} fontSize="9" fill="var(--cs-ink-dim)">{row.label}</text>
